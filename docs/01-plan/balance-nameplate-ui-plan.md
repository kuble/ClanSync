...
- **A점수**(자동 추정 값, 서버·문서에서는 AI 파이프라인으로 산출): 화면에는 **「A점수」** 라벨 + 숫자로 표기. **Premium에서만** 노출. Free에서는 **완전 비표시** (`body.mock-plan-free` + `mock-hide-on-free`).
...
- **닉네임·오늘 전적·Manual·A점수**는 슬롯 **왼쪽 본문 열**에 둔다. **A점수는 M(Manual) 입력 바로 아래** 한 줄로 둔다.  
...
1. **팀 배치 확정 시** (`배치 완료` / 실제로는 라인업·시너지 계산에 필요한 스냅샷이 고정된 시점): **반드시 1회** A점수(자동 추정) 스냅샷 갱신.
...
- `.mock-balance-slot-line2` — 전적 + `.mock-balance-slot-scores-col`(세로: `M` 입력 → `.mock-balance-slot-a-wrap`의 **A점수**). **태그는 여기 넣지 않는다.**
...
- Manual·A점수·태그·전적은 **API 스냅샷**으로 내려받고, 클라이언트에서 **자정 기준 임의 재계산**하지 않는다.
- Free: A점수 필드 미수신 또는 `null` → UI 비노출.
