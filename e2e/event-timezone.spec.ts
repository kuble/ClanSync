...
test("R06/R07 KST create, title edit and time edit preserve UTC instants and reservations", async ({ page }) => {
  ...
  try {
    ...
    await expect(page.getByText("일정을 추가했습니다.", { exact: true })).toBeVisible();
    const { data, error } = await svc.from("clan_events").select("id,start_at").eq("clan_id", clanId).eq("title", title).single();
    ...
    if (!(await dayCell.count())) await page.getByRole("button", { name: "다음 달" }).click();
    await dayCell.click();
    await page.getByRole("button", { name: new RegExp(title) }).click();
    await page.getByRole("button", { name: "편집", exact: true }).click();
    await page.locator("#edit-title").fill(`${title}-edited`);
    await page.getByRole("button", { name: "저장", exact: true }).click();
    await expect(page.getByText("일정을 수정했습니다.", { exact: true })).toBeVisible();
    const firstEdit = await svc.from("clan_events").select("start_at").eq("id", eventId!).single();
    ...
    await expect.poll(async () => {
      const r = await svc.from("clan_events").select("start_at").eq("id", eventId!).single();
      return new Date(r.data!.start_at).toISOString();
    }).toBe(`${date}T12:30:00.000Z`);
    const logs = await svc.from("notification_log").select("id,status,scheduled_at").eq("event_id", eventId!);
    ...
    const cleanup = eventId
      ...
      : svc.from("clan_events").delete().eq("clan_id", clanId).eq("title", title);
