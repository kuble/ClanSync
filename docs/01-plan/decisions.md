...
|------|------|------|-----------------------|
| D-UX-01 | OPEN · 수동 UI 확정 (2026-09-16) | 게임 대기방 기준 내전 운영·구성원 조작 최소화 | **수동 명단 UI/UX 확정:** 최근 참여순 직접 선택·좌우 고정 역할·첫 빈칸 자동 입력·우클릭 삭제·선택자 숨김·직접 드래그 이동/교환·우상단 되돌리기/리셋 아이콘. 관전자·체크·범례·별도 모드·복구 완료/예시 명단 문구 제거. **운영:** 10명 유지·라운드 사이 교체. 대기방 화면 연결 적용·게임사 문의 후속(페이지·S04 반영). **미결:** 추가 편의성 후보·모바일·저장·OCR·플랜·PC 보조앱·밴픽·계정 요건. 실제 앱 적용·게임사 정책 적합성 검증 미완료. 기존 D-EVENTS-01·D-PERM-01 유지 / [논의](./product-design-evolution.md#ux-01) |
| D-UX-02 | OPEN (2026-09-16) | 역할별 홈·새 클랜 첫 사용·초대 진입 | D-AUTH-01·D-SHELL-01·D-PERM-01 / [논의](./product-design-evolution.md#ux-02) |
| D-UX-03 | OPEN · 역할 선고정 확정 (2026-09-16) | 편성 방식 선택·선호 역할 추첨·주장 지명·경매·공유 연출 | **확정:** 역할 추첨 대상은 이미 정해진 10명, 주장 지명·경매는 역할을 먼저 확정하고 팀원을 선발. **기준안 v0.1:** 네 방식의 상세 규칙·운영진/공유 화면·연출·정정. 불가 역할·주장 선정·순번·시간·예산·무입찰·입력 주체·다팀 범위·기존 점수 공개/용어는 미결. 대화 내 연출 시연이며 앱 구현 전. 밸런스 명세·D-PERM-01·D-PRIV-01 / [논의](./product-design-evolution.md#ux-03) · [상세 기준안](../02-design/formation-modes.md) |
...
| 코드 | 상태 | 항목 | 결정 / 영향 문서 |
|------|------|------|------------------|
| D-SESSION-01 | DECIDED (2026-09-16) | 세션·라운드·내전 날짜·사이트 참여 정의 | **세션 = 운영진이 여는 당일 내전 전체, 라운드 = 경기 한 판.** 세션을 열면 클랜원이 사이트에서 실시간 참여하고 기능별 권한에 따라 조작·베팅할 수 있다. 내전 날짜는 세션 개설일로 고정하며 종료 시점까지의 라운드·회차를 해당 세션에 기록한다. 라운드 출전자 10명과 세션 참여자는 구분한다. 상세 단일 기준: [용어 사전](./glossary.md#내전-운영-용어). KST 06:00을 내전 날짜로 쓰던 표현은 이 결정으로 대체한다. 기존 권한·플랜과 별도 일일 집계 정책은 유지. PRD·밸런스 페이지/S04·권한 표·편성 기준안에 반영, 실제 앱·DB 반영 및 검증은 별도. |
...
| D-EVENTS-05 | DECIDED (2026-04-20), 배치 변경 (2026-09-24) | 대진표(Bracket) 결과의 클랜 통계·코인 반영 여부 | **대진표 = 클랜 내 이벤트**(클랜 간 토너먼트는 기획 없음). 정규 내전과 별도 집계하며 화면 배치는 D-STATS-05 이후 후속 설계. HoF·외부 순위표 반영 X. 이벤트 포함 활동 지표와 내전 전적은 구분한다. **코인은 D-ECON-01 확정값만**(개최 500 차감·참가 +200·우승 +1,000, 전부 클랜 풀). 개인 풀 보상·MVP 자동 산정 **없음**. [§D-EVENTS-05](#d-events-05--대진표-결과의-통계코인-반영) |
...
| D-STORE-04 | DECIDED (2026-09-25) · 구현 예정 | 클랜 테마 판매와 적용 | 클랜 스토어에서 테마 구매 → 보유 테마 선택 → 해당 클랜 페이지에 즉시 적용. [§D-STORE-04](#d-store-04--클랜-테마-구매선택즉시-적용) |
...
| D-STATS-03 | DECIDED (2026-04-21), 배치·권한 변경 (2026-09-24) | 앱 이용 측정 단위 | 활동일(person-day) 정의 유지. 사이트 방문 통계는 클랜 관리·운영진 전용으로 이동하고 내전 참여와 분리(D-STATS-05). [§D-STATS-03](#d-stats-03--앱-이용-횟수-측정-단위--활동일-person-day) |
...
| D-STATS-05 | DECIDED (2026-09-24), 구현 대기 | 통계 3영역과 역할별 관계 전적 | 명예의 전당·내전 통계·개인 기록으로 재편. 앱 이용은 관리로 이동. 기준 선수 역할 × 상대 역할 × 아군/적군 교집합으로 전적 집계. [§D-STATS-05](#d-stats-05) |
| D-STATS-06 | DECIDED (2026-09-24), 구현 대기 | 통계 항목별 UI/UX와 탐색 우선순위 | 핵심 숫자 → 변화·비교 → 근거 경기. 영역별 차트·표·상세 탐색, 역할 조합표, 필터 복원·접근성·데이터 상태를 명세화. [§D-STATS-06](#d-stats-06) |
...
| D-NOTIF-03 | DROPPED (2026-04-21) | 이메일 다이제스트(일간/주간 요약) 도입 여부 | **도입하지 않음**. 이메일은 **거래 메일(비밀번호 재설정·가입 인증·필수 공지)로만** 사용. 재참여·이탈 방지 유인은 in-app 피드(D-NOTIF-01)·브라우저 푸시(D-NOTIF-02)로 갈음. 결정 번복 시 D-NOTIF-03b로 재오픈. [§D-NOTIF-03](#d-notif-03--이메일-다이제스트-dropped) |
...
| 잠금 시간 | **15분**. 만료 후 첫 실패는 새 연속 실패 1회로 집계 |
...
### D-STORE-04 — 클랜 테마 구매·선택·즉시 적용

- **결정일**: 2026-09-25
- **상태**: 기획 확정, 구현 예정. 현재 웹페이지의 테마는 변경하지 않는다.
- **요지**: 클랜 테마를 클랜 스토어의 꾸미기 상품으로 판매한다. 구매가 완료되면 해당 클랜의 보유 테마가 되며, 운영진이 보유 테마를 선택하면 그 클랜의 홈과 하위 페이지가 모든 방문자에게 즉시 새 테마로 보인다.
- **구매와 적용 분리**: 구매만으로 활성 테마는 바뀌지 않는다. 보유 테마 간 전환과 기본 테마 복귀에는 재구매나 추가 차감이 없다. D-STORE-03의 환불 불가 원칙은 테마 소유권에 적용한다.
- **범위**: 테마는 클랜 단위 설정이다. 랜딩 페이지, 다른 클랜, 개인 프로필에는 적용하지 않는다. 구매 권한과 클랜 풀 지출 제한은 D-STORE-01·D-ECON-02를 따른다.
- **첫 시안**: 이번 대화에서 확정한 포털 콘셉트(차콜 격자 배경, 베이지 선과 글자, 절제된 하늘색 포인트). `c6097ad` 커밋의 시안 구현을 참고하되, 실제 테마 기능은 추후 개발한다. 카탈로그와 가격은 구현 시 결정한다.
...
- **결정일**: 2026-04-20
- **요지**: **외부 공개 클랜 순위표**에서 승률·K/D·MVP 수 등 **경쟁 지표를 전면 제외**한다. 친목·즐겜·빡겜·프로 등 다양한 지향(`clans.style`)을 허용하는 서비스 정책과 정합하며, 경쟁 지표로 인해 친목 지향 클랜이 위축되거나 허수 기록 욕구가 생기는 것을 구조적으로 방지한다. 클랜 **내부**의 경쟁 지표는 [D-STATS-05](#d-stats-05)의 HoF 부분 공개·개인 기록·정보별 권한을 따른다(2026-09-24 변경). 외부 클랜 순위표 제한과 별도 `expose_hof` 정책은 유지한다.
...
- **결정일**: 2026-04-20
- **요지**: **대진표는 클랜 내 이벤트** 전용(클랜 간 토너먼트는 기획 범위 밖). 통계는 정규 내전과 **완전 분리해 집계**하고 화면 배치는 D-STATS-05 이후 후속 설계한다. **코인은 D-ECON-01 확정값**(개최 500·참가 +200·우승 +1,000, 전부 **클랜 풀**)만. 개인 풀 보상·MVP 자동 산정 **없음**. 기존 동일 가중치 참여·매너 지표는 이벤트를 포함한 활동 지표이며 새 내전 참여 세션 수·출전·승률의 분모에 섞지 않는다.

**통계 반영 매트릭스**

| 지표 | 정기 내전(`intra`) | 대진표(`bracket`) | 비고 |
|------|:-----------------:|:-----------------:|------|
| 경기 전적 | ✓ | ✗ (별도 집계) | 외부 경쟁 순위표 비노출(D-ECON-03). 클랜 평균 승률을 핵심 KPI로 사용하지 않고 K/D는 수집 전 미제공(D-STATS-05) |
| 이벤트 포함 참여 활동 (개인·클랜) | ✓ | ✓ (동일 가중치) | 기존 한 경기 1포인트 기획. D-STATS-05의 내전 참여 세션 수·출전과 별개 |
...
| 클랜 통계(`/stats`) | 대진표 결과는 내전 표본과 별도 집계. 기존 별도 상단 탭 신설안은 D-STATS-05의 3영역 재편에 따라 후속 배치를 별도 설계한다. |
...
- [pages/10-Clan-Stats.md — 내전 통계·경기 아카이브](./pages/10-Clan-Stats.md)
...
- **결정일**: 2026-04-21. **배치·열람 정책 변경: 2026-09-24 D-STATS-05.**
- **유지하는 정의**: 인증된 멤버가 자기 클랜 페이지에 처음 방문한 날 = 활동일 1회. 같은 날 추가 접속·새로고침은 더하지 않는다. 월 활동일은 모든 멤버의 person-day 합이다.
- 기존 통계의 `앱 이용` 탭과 전 멤버 열람 기획은 폐기한다. **클랜 관리 → 개요 → 사이트 이용 통계**, leader·officer 열람으로 대체한다. 사이트 방문자와 내전 출전 인원을 분리한다.

| 항목 | 기준 |
|---|---|
| 기록 단위 | `(clan_id, user_id, activity_date)` UNIQUE. 같은 날 중복 적립 방지 |
| 트리거 | 본인 소속 클랜 컨텍스트 페이지의 첫 실제 방문. 클랜 메인·관리·통계·일정 등 포함 |
| 제외 | 비인증·타 클랜 탐색·클랜 비종속 허브/프로필·봇·프리페치 요청 |
| 날짜 | 클랜 시간대(없으면 Asia/Seoul)의 자정 경계. 내전 날짜·코인 일일 경계와 별개 |
| 활동일 | 기간 내 기록 행 수. 월별 합으로 연간 합 산출 가능 |
| 방문 순인원 | 기간 내 방문한 `user_id`의 distinct. 월별 distinct의 합은 연간 distinct가 아님 |
| 내전 순출전 인원 | 방문 테이블이 아닌 유효 경기 출전 명단의 distinct. **내전 통계**에서 제공 |
| 외부 공개 | 다른 클랜·비멤버에 노출하지 않음(D-ECON-03) |

**수집·조회 정책**

- 원본은 `clan_daily_member_activity`. 본인이 해당 클랜 활성 멤버일 때만 당일 기록을 적립하며 과거·미래 날짜 및 타 클랜 적립을 차단한다.
- 모든 멤버의 적립은 유지한다. **조회는 운영진으로 제한하는 것이 새 목표 정책**이며 화면 이동과 함께 서버 조회·RLS·집계·CSV 경로를 검증해야 한다. 이 문서 변경은 DB 마이그레이션 적용이 아니다.
- 중복 INSERT는 무시하고 일반 사용자의 UPDATE/DELETE는 허용하지 않는다. 탈퇴 시 과거 집계 보존과 계정 삭제 정책은 기존 기준을 따른다.
- 월 `person_days`·`active_members`와 연간 distinct는 각각 올바른 기간으로 집계한다. `active_members`라는 기존 식별자는 **사이트 방문자**이며 경기 참여자를 뜻하지 않는다.
- CSV는 원본 열람 권한과 `export_csv`를 모두 확인한다. CSV 권한만으로 관리 통계 접근을 열지 않는다.

과거 Phase 1의 정적 3영역 화면과 MV/cron 제안은 당시 구현 메모다. 현행 상태는 코드·마이그레이션을 기준으로 확인하며, 추가 기능별 이벤트 집계는 별도 수집 설계 후 도입한다.

**연관:** [클랜 관리](./pages/12-Clan-Manage.md) · [스키마](./schema.md) · [D-STATS-05](#d-stats-05) · [D-PERM-01](#d-perm-01--클랜-권한-매트릭스-모델-도입)

---

### D-STATS-04 — CSV 내보내기 (D-PERM-01 흡수 + Phase 2+ 구현 보류)

- **결정일**: 2026-04-21. **화면 참조 갱신: 2026-09-24 D-STATS-05.**
- CSV 권한은 `export_csv`: 기본 leader, officer는 클랜 위임 시 허용. 권한 등록과 실제 CSV 기능 구현은 별도다.
- 도입 대상: **클랜 관리의 이용 통계**, **내전 통계의 경기 아카이브**, **명예의 전당의 등재 명단**. 각 원본의 기간·열람 범위·개인 비공개 정책을 그대로 적용한다.
- 기존 `앱 이용 탭 / 경기 기록 탭` 배치 참조는 D-STATS-05로 대체한다. 이번 문서화에 CSV 구현은 포함하지 않는다.
- **연관:** [D-PERM-01](#d-perm-01--클랜-권한-매트릭스-모델-도입) · [클랜 통계](./pages/10-Clan-Stats.md) · [클랜 관리](./pages/12-Clan-Manage.md)

---

<a id="d-stats-05"></a>

### D-STATS-05 — 통계 3영역 재편과 역할별 함께한 기록·상대 전적

- **결정일:** 2026-09-24, 사용자 요청에 따른 기획 대체. **문서화 완료·구현 대기**.
- **기준 명세:** [클랜 통계 기획](./pages/10-Clan-Stats.md). 기존 목업 중심 4탭 기획을 대체한다.

| 영역 | 책임 |
|---|---|
| 명예의 전당 | 월·연·전체의 주요 기록 순위·역대 등재. 운영진은 전체 집계, 멤버는 설정된 항목·공개 시점·상위 범위 |
| 내전 통계 | 경기·참여·맵·밴·편성·균형·경매 분석과 경기 아카이브 |
| 개인 기록 | 본인의 전적·역할·맵·점수·참여·예측·수상·경기 목록. 다른 멤버 선택 시 기존 정보별 권한 적용 |
| 클랜 관리 → 개요 → 사이트 이용 통계 | 기존 사이트 방문·활동일·방문 순인원. 운영진 전용. 경기 참여 지표는 내전 통계에 유지 |

**역할별 관계 기록**

- 개인 기록의 함께한 기록·상대 전적은 **기준 멤버의 실제 경기 역할 × 상대 멤버의 실제 경기 역할 × 아군/적군**을 독립 선택한다. 양쪽 역할은 각각 전체·돌격·공격·지원이다.
- 현재 프로필·선호 역할이 아닌 경기 당시 로스터 역할로 분류한다. 공격/지원의 1·2 슬롯은 같은 역할이다.
- 상대별 경기 수·승·무·패·승률을 **기준 멤버 관점**으로 표시한다. 적군은 그 멤버를 상대로 기준 멤버가 이긴 비율이다. 한 경기·같은 상대는 1회, 본인은 제외한다.
- 모든 필터는 교집합으로 적용하며 행 선택 시 집계 근거인 경기 목록으로 연결한다. 역할 미상은 해당 역할의 `전체`에서만 포함할 수 있고 특정 역할로 추정하지 않는다.
- 예: 기준 멤버가 공격, 아군이 지원인 경기 2승 1무 1패 → **4경기·50%**. 두 사람이 다른 역할·적군으로 만난 경기는 별도 조건에서 집계한다.

**집계·권한 경계**

- 정규 내전만 영구 집계한다. 깜짝 내전 제외. 유효 출전은 승·무·패가 확정된 라운드이며 무효·미정은 제외한다.
- 승률은 기존 세션 결과 정책과 같은 **승/(승+무+패)**. 한 세션에서 5경기 출전한 경우 참여 세션 1회·출전 5경기로 구분한다. 과거 역할·세션 누락을 임의로 보간하지 않는다.
- 경기 기간은 D-SESSION-01의 내전 날짜를 사용한다. 사이트 방문 날짜는 D-STATS-03의 별도 기준이다.
- D-STATS-01의 HoF 설정, D-STATS-02의 정정·이력, D-STATS-03의 활동일 측정, D-STATS-04의 CSV 권한을 유지한다. **D-STATS-03의 전 멤버 방문 통계 열람만 운영진 전용으로 변경**한다.
- 본인 개인 기록과 타인 상세 열람을 구분한다. 타인 통계는 기존 통계 5개 권한과 D-PRIV-01, 경기 상세는 `view_match_records`를 적용한다. 관계별 수치는 양쪽 선수 접근 범위를 확인하며 본인 화면이 타인 비공개를 우회하지 않는다.
- D-ECON-03의 외부 클랜 순위표 제한·leader의 별도 `expose_hof`는 유지한다. 클랜 내부는 본 결정의 HoF 부분 공개·개인별 접근 범위를 따른다.
- D-EVENTS-05의 대진표 별도 집계 원칙은 유지한다. 별도 상단 탭 추가 계획은 이 3영역에 포함하지 않으며 후속 배치를 별도 설계한다.
- Free/Premium에 새 통계 제한을 신설하지 않는다. 승부예측은 기존 Premium·사용 설정·정산 정책을 따른다.

최소 관계 표본·예측 등재 표본·새 순위 동률 규칙은 구현 전 조정 항목이다. 실제 영웅 사용·KDA·숙련도 등 없는 데이터는 생성하지 않는다. 새 UI·집계·권한 변경의 수용 기준과 현재 구현 차이는 기준 명세에 남긴다.

---

<a id="d-stats-06"></a>

### D-STATS-06 — 통계 항목별 UI/UX와 탐색 우선순위

- **결정일:** 2026-09-24, 사용자 승인에 따른 기획 반영. **UI/UX 문서화 완료·구현 대기**.
- **기준 명세:** [클랜 통계 §8](./pages/10-Clan-Stats.md#stats-ui-ux). 항목별 표현·인터랙션·레퍼런스의 단일 기준으로 사용한다.
- **공통 흐름:** 핵심 숫자 → 변화·비교 → 실제 근거 경기. 중요한 운영 지표를 먼저 배치하고, 대표 그래프·짧은 목록에서 상세를 연다.

| 영역 | 우선 배치와 표현 |
|---|---|
| 명예의 전당 | 최다 참여·다승·승률 대표 기록과 부문을 전환하는 하나의 순위 영역. 출전·표본·등재 조건을 함께 표시 |
| 내전 통계 | 개최 세션·완료 경기·출전 멤버 3개 숫자 → 참여 추이 → 편성 균형 → 최근 경기. 맵·밴·경매는 주제별 상세 |
| 개인 기록 | 전적·최근 흐름 → 역할별 기록 → 함께한 기록·상대 전적. 맵·참여·점수·예측·수상·경기 목록으로 확장 |
| 관리의 사이트 이용 통계 | 방문 규모 → 추이 → 월·연 비교. 활동일은 보조 지표, 그래프·표 전환 제공 |

- 역할별 관계 기록은 **기준 멤버 역할·같은 팀/상대 팀·상대 역할**을 문장으로 읽히는 필터와 정렬 가능한 표로 표현한다. 추가 3×3 역할 조합표의 칸을 선택하면 양쪽 역할과 근거 목록을 연결한다. 경기 수·승무패·승률을 함께 표시하고 적군 전적을 1:1 결투 승률로 오해하게 하지 않는다.
- 차트 선택은 해당 조건의 기록으로, 멤버 선택은 허용된 개인 기록으로 연결한다. 기간·필터·정렬·스크롤을 복원하고 조건별 해제·초기화를 제공한다. 상세 패널은 중첩하지 않는다.
- 호버는 짧은 수치 확인에 한정하며 클릭·터치·키보드로도 상세에 접근한다. 반응형, 색 외 문자·아이콘, 동작 줄이기, 기간 선택의 드래그 대안을 포함한다.
- 이전 동일 길이 기간의 비교 날짜와 진행 중 집계를 명시한다. 비율의 분모·단위를 표시하고 로딩·오류·비공개·미수집·무기록·소표본을 구분한다. 편성 점수 차이는 경기 전 점수의 차이이며 실제 접전·공정성을 뜻하지 않는다.
- D-STATS-05의 3영역·명예의 전당 기본 진입, 정규 내전 집계·역할 기준·정보별 권한을 유지한다. 차트나 연결 상세가 공개 범위를 넓히지 않는다. 미수집 이력은 추정값으로 채우지 않는다.
- Plausible·Metabase·OP.GG·FACEIT Track·GitHub·Nivo·Recharts의 확인한 패턴과 ClanSync 채택 범위를 기준 명세에 기록한다. 차트 라이브러리 도입은 후속 구현에서 결정한다.

화면 우선순위와 실제 구현 순서는 구분한다. 데이터 준비·집계·권한·화면 동작은 후속 구현 및 검증 대상이며, 이번 문서화로 수용 기준을 완료 처리하지 않는다.
...
6. ~~**후속 결정 후보 D-NOTIF-03** — 이메일 다이제스트(일간/주간 요약) 여부.~~ → **DROPPED 2026-04-21** (도입 보류). 재오픈은 D-NOTIF-03b 조건 충족 시. [§D-NOTIF-03](#d-notif-03--이메일-다이제스트-dropped)
...
### D-NOTIF-03 — 이메일 다이제스트 (DROPPED)

- **결정일**: 2026-04-21 (사용자 컨펌)
- **상태**: **DROPPED** — 도입하지 않음.
- **요지**: 일간·주간 이메일 다이제스트(한 주간 활동·놓친 알림 요약 메일)는 **필요성 부족**으로 도입 보류. 거래 메일(비밀번호 재설정·가입 이메일 인증·필수 공지·약관 변경 고지 등)은 다이제스트와 별개로 **유지**.

**DROPPED 근거**

| # | 근거 | 세부 |
|---|------|------|
| 1 | 재참여·이탈 방지는 in-app + 웹 푸시로 갈음 | D-NOTIF-01(in-app 피드)·D-NOTIF-02(브라우저 푸시, Premium 전용)로 **앱 생태계 안에 있는 사용자**는 충분히 커버. 다이제스트의 주 가치인 "앱을 떠난 사용자 재방문"은 타깃이 분명하지 않고 ROI 검증 전 |
| 2 | 스팸 인식 리스크 | 게임 커뮤니티 특성상 "클랜 앱이 매주 메일을 보낸다"는 감각은 스팸으로 분류될 가능성. Gmail 스팸 탭 이동 시 거래 메일(비밀번호 재설정 등) 배달률 동반 저하 위험 |
| 3 | 법적·운영 부담 | 정보통신망법 제50조 옵트인 관리, 야간 광고 금지(21~08 KST) 타이밍 제약, 1-Click Unsubscribe(RFC 8058) 헤더 구현, Bounce/Complaint 처리 파이프라인, 발송 공급자 계약(Resend·SES·SendGrid) — Phase 1~2 범위에서는 오버헤드 |
| 4 | 발송비 불확실성 | 멤버 수 × 주간 발송으로 월 수만 건 단위. 공급자별 무료 한도(Resend 3k/월·SES 62k/월) 초과 시 비용 발생. 실제 오픈률·클릭률 데이터 부재 상태에서 지출 정당화 곤란 |
| 5 | 피벗 여지 보존 | 운영 6~12개월 누적 후 이탈률·복귀 경로 데이터 확보 시 **D-NOTIF-03b로 재오픈** 가능. 지금은 결정을 고정하지 않음 |

**번복(재오픈) 조건**

다음 중 하나라도 명확해지면 D-NOTIF-03b로 재검토:

- 운영 3~6개월 후 **비활성·휴면 복귀율 측정값**이 낮고(<5%), 경쟁사가 유사 기능으로 복귀율 2배 이상 확보하는 사례가 보일 때.
- 클랜장 인터뷰에서 "주간 요약 메일이 있으면 좋겠다"는 요구가 **3개 이상 독립 소스에서 제기**될 때.
- Premium 요금제 가치 제안 강화가 필요하고, 카카오·Discord·web_push만으로 차별화 부족이 수치로 드러날 때.

**남는 것 / 유지되는 것**

- **거래 메일**(비밀번호 재설정·이메일 인증·필수 공지·약관 변경 고지)은 계속 발송. 사용자 옵트아웃 불가(필수 거래성).
- 공급자는 Phase 2+ 거래 메일 구현 시점에 결정(**D-EMAIL-01** 신규 후속 후보).
- `users` 테이블에 **다이제스트 구독 컬럼을 미리 추가하지 않음** — 재오픈 시 `email_digest_subscriptions` 테이블 신설로 충분.
- `notification_log.channel` enum은 `email_digest` 값을 **추가하지 않음**. 재오픈 시 enum 확장.

**영향 정리 (변경 없음)**

- 스키마: 변경 없음.
- 목업: 변경 없음 (R3 예고 배너 신설 안 함).
- 페이지 문서: 변경 없음 (벨 드로워 섹션·프로필 알림 설정 섹션 모두 현 상태 유지).
- 권한 매트릭스(D-PERM-01): 영향 없음.

**후속 결정 후보**

- **D-EMAIL-01** (OPEN, Phase 2+) — 거래 메일 공급자 선택 및 템플릿 정책 (Resend · AWS SES · SendGrid 등). 다이제스트와 분리.
- **D-NOTIF-03b** (OPEN, 조건부) — 다이제스트 도입 재검토. 위 "번복 조건" 충족 시에만 재오픈.
...
| `view_synergy_winrate` | 개인 기록의 함께한 기록·상대 전적(양쪽 실제 역할 × 아군/적군, D-STATS-05) 및 편성의 관계별 수치 |
