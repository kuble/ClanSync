...
**2026-09-24 화면 피드백 반영:** 사용자에게 보이는 `세션`은 `내전`으로 표기한다. 출석·출전·승률·맵·영웅 밴·개인 역할/맵 전적에는 분모가 명확한 게이지를 적용한다. 게이지는 별도 막대가 아니라 해당 행·카드 배경 전체를 왼쪽부터 비율만큼 채우고, 이름·수치는 그 위에 표시한다. 최장 연승에는 임의 비율을 만들지 않는다. 편성 점수 차이는 관리 화면에서만 전달·표시한다. 예측은 기존 적중 횟수 순위를 유지하고, 행의 게이지로 적중률을 함께 설명한다.

**2026-09-25 화면 피드백 반영:** 명예의 전당 상단 대표 버튼과 부문 선택기를 제거한다. 승률·최다 출석·최다 출전·예측 적중 순위를 각자 한 열짜리 영역으로 표시하며 데스크톱에서는 2×2로 배치한다. 각 행은 개인 기록으로 연결한다. 최장 연승 순위의 기존 집계·공개 설정 값은 보존하되 이 네 영역에는 표시하지 않고, 개인 기록의 연승 지표는 유지한다.
...
| 화면 | 첫 화면의 순서 | 추가 탐색 |
|---|---|---|
| 명예의 전당 | 승률·최다 출석·최다 출전·예측 적중 순위 네 영역 | 기간 전환, 순위 행에서 개인 기록·역대 등재 |
| 내전 통계 | 핵심 숫자 3개 → 월별 추이 → 맵·밴 → 경기 기록 | 맵·밴과 날짜·맵·참가자 조건으로 경기 탐색. 맵 투표·경매 집계는 클랜 관리 개요에서 확인 |
...
통계 부문·기간·월/연도·역할·정렬 등 선택형 필터는 같은 탄성 세그먼트 방식으로 전환한다. 긴 월 목록은 연도와 월을 따로 선택하고, 좁은 화면에서는 세그먼트 내부를 가로로 스크롤한다. 키보드 방향키와 동작 줄이기 설정을 지원한다.
...
| 참여·경기 추이 | 월별 꺾은선 그래프. 출전 인원·개최·경기 전환 | 연도·월 세그먼트 선택 → 해당 월 경기 기록 필터 |
