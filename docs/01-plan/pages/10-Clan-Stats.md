...
맵 유형 필터는 쟁탈·밀기·화물·혼합·플래시포인트 아이콘을 사용한다. 역할 아이콘은 Blizzard 공식 홈페이지/CDN 원본, 맵 유형 기호는 자체 제작 SVG이며 출처는 `public/images/overwatch/icons/SOURCES.json`에 기록한다. 플래시포인트의 수라바사·뉴 정크 시티·아틀리스는 맵 목록·수동 선택·투표 후보에서 지원한다. 기록이 없는 유형에 임의 통계는 생성하지 않는다.
