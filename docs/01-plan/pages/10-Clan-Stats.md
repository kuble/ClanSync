...
**2026-09-24 · D-STATS-05·06 · 정보 구조·UI/UX 기획 확정, 구현 대기.** 기존 `요약 / 명예의 전당 / 경기 기록 / 앱 이용` 4탭 기획을 이 문서로 대체한다. 이번 변경은 문서화만 해당하며 화면·집계·DB 권한이 반영됐다는 뜻은 아니다.
...
| 화면 | 첫 화면의 순서 | 추가 탐색 |
|---|---|---|
| 명예의 전당 | 최다 참여·다승·승률 대표 기록 → 선택한 부문의 순위 목록 | 부문·기간 전환, 개인 기록·역대 등재 |
| 내전 통계 | 핵심 숫자 3개 → 참여 추이 → 편성 균형 → 최근 경기 | 맵·밴·편성 방식·경매의 주제별 요약에서 상세 열기 |
...
상단의 최다 참여·다승·승률 대표 기록은 작게 배치한다. 아래 **하나의 공통 순위 영역**에서 부문을 바꾸며, 멤버에게는 공개된 부문·순위만 표시한다. 대표 기록도 동일한 공개 시점·등재 기준을 따른다. 비공개 부문의 자리나 등재자가 없는 자리를 샘플 수상자로 채우지 않는다.

| 항목 | 표현 | 인터랙션 |
|---|---|---|
| 승률 순위 | 아바타·닉네임·승률을 강조한 목록. 출전 경기 수와 승·무·패 병기 | 멤버 선택 → 같은 기간 개인 기록. 등재 기준 설명 제공 |
| 다승 순위 | 이름·승수를 직접 표시한 가로 막대 순위 | 행 선택 → 해당 멤버의 승리 경기 목록 |
| 최다 출전 | `32경기 출전`처럼 경기 단위를 명시한 순위 목록 | 선택 → 출전 경기 목록 |
| 최다 참여 | `8개 세션 참여`처럼 출전과 구분한 수치·참여 날짜 | 선택 → 참여 세션 달력 |
| 최장 연승 | 연승 수·달성 기간과 작은 승리 기록 띠 | 선택 → 연승을 구성한 경기 순서 펼치기 |
| 승부예측 | 적중률과 적중/유효 예측 수 병기 | 적중률·적중 횟수 전환. 허용된 정산 기록으로 연결 |
| 역대 등재 | 월·연도별 기록 보관 목록. 기간별 대표 멤버·부문 표시 | 기간 선택 → 당시 명예의 전당 |

위 숫자는 표현 예시다. 왕관·메달은 상위 기록에 제한적으로 사용하고, 부문 전환·순위 변경은 짧은 전환으로 표현한다. 순위가 실시간 갱신돼 읽던 행이나 포커스가 갑자기 바뀌지 않도록 한다.

### 8.3 내전 통계 표현

운영자가 **얼마나 열렸는지, 누가 출전했는지, 편성이 어떤 상태였는지**를 먼저 판단하게 한다. 대표 숫자는 `개최 세션 / 완료 경기 / 출전한 멤버`의 3개다. 숫자 선택으로 아래 대표 그래프의 지표를 전환한다.

| 항목 | 표현 | 인터랙션 |
|---|---|---|
| 세션 수·완료 경기·순출전 인원 | 단위가 분명한 핵심 숫자 3개 | 선택한 지표로 대표 그래프 전환 |
| 세션당 평균 경기·출전 인원 | 관련 핵심 숫자 아래의 작은 보조 수치 | 평균 계산 대상·분모 확인 |
| 참여·경기 추이 | 주별 세로 막대. 출전 인원·개최·경기 전환 | 막대 선택 → 해당 기간 세션 목록. 이전 기간 비교 |
| 편성 점수 차이 | 팀 점수 합계 차이가 작은 경기부터 큰 경기까지의 분포와 대표 수치 | 평가·분석 점수 전환. 차이 구간 선택 → 실제 경기 |
| 예상 우세 팀의 실제 결과 | 예상 우세 팀의 승리 수/경기 수와 승·무·패 비율 띠 | 편성 방식별 조회. 예측을 확인할 수 있는 경기로 연결 |
| 최근 경기·아카이브 | 날짜·세션으로 묶인 목록. 맵 썸네일·결과·참가자 요약 | 행 펼치기 → 양 팀 명단·역할·밴. 날짜·맵·멤버 검색 |
| 편성 방식 | 직접·추첨·주장·경매의 가로 비율 띠와 경기 수 | 방식 선택 → 같은 방식의 균형·경기 기록 |
| 같은 팀·상대 팀 빈도 | 멤버 선택 시 자주 만난 멤버 목록 | 같은 팀·상대 팀 전환 → 개인의 관계 기록 |
| 요일·시간대 | 요일 × 시간대 색상표. 많이 열린 구간을 진하게 표시 | 칸 선택 → 해당 시간대 세션 |
| 맵·유형별 빈도 | 맵 썸네일과 가로 막대 목록 | 유형 필터, 진행 횟수순·이름순 정렬, 맵별 기록 열기 |
...
| 참여 기록 | 월별 참여 달력. 참여한 날짜의 세션 수 표시 | 날짜 선택 → 그날 출전 세션·경기 |
...
| 전체 경기 목록 | 최신순 목록, 세션별 묶음 | 검색·필터·상세 탐색 후 위치 복원 |
...
`클랜 관리 → 개요 → 사이트 이용 통계`에서 방문자 수·활동일 보조 수치 → 대표 추이 → 월·연 비교 순서로 배치한다. [관리 명세의 항목별 표현](12-Clan-Manage.md#site-usage-ui)을 단일 기준으로 사용한다. 경기 참여 달력과 사이트 방문 달력은 데이터·라벨을 구분한다.
...
5. **쉬운 문장과 단위:** `이번 달 18명이 출전했습니다`처럼 수치를 설명한다. 참여는 세션, 출전은 경기, 방문은 사이트 이용으로 구분한다. 비율에는 표본·분모·기간을 병기한다.
...
| [GitHub 활동 기록](https://docs.github.com/en/account-and-profile/how-tos/contribution-settings/viewing-contributions-on-your-profile) | 활동 날짜를 선택해 관련 기록 탐색 | 참여 날짜에서 세션으로 이어지는 달력 |
| [Nivo Calendar](https://nivo.rocks/calendar/) · [Heatmap](https://nivo.rocks/heatmap/) | 달력·행열 조합 시각화 예시 | 참여 달력과 추가 보기인 역할 조합표 |
...
- [ ] 내전 통계 첫 화면에서 핵심 숫자·참여 추이·편성 균형·최근 경기를 우선 확인한다. 공개 범위와 데이터 준비 상태를 따른다.
