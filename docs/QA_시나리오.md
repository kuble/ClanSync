...
**한 줄 요약**: ClanAuth 헤더·카드 상태·프로필 요약 및 목록이 모두 신청 상태와 맞춰 보이고, 같은 게임에 이미 신청했을 때 일반 「가입 신청」 버튼이 그대로 뜨지 않는지 확인한다.
...
4. 같은 페이지에서 클랜 A 카드에는 **신청 대기 중** 배지만 있고 「가입 신청」 버튼이 없는지 본다.
5. 같은 게임 목록 안의 다른 클랜 **B** 카드에는 「가입 신청」 대신 「**이 클랜으로 신청 바꾸기**」(또는 유사 라벨)만 있는지 확인한다(클릭 시 교체 확인 다이얼로그).
6. `/profile` 로 이동해 헤더 아래 「**진행 중인 가입 신청**」 요약과 「가입 신청」 목록에 동일 신청(A)이 표시되는지 본다.

**기대 결과**:

- [ ] ClanAuth 헤더·카드 UX가 위 행위와 같다.
- [ ] 프로필에서 요약 + 상세 목록에 `pending` 이 보인다.
- [ ] 이미 신청한 클랜 제외 카드에서는 보내기 전부터 중복 「가입 신청」 버튼이 숨겨져 있다.

**깨졌을 때**: `clan_join_requests` 본인 `pending`, `/profile` 로그 `[profile] clan_join_requests` 메시지, Network(Server Actions)·RLS·embed 없는 조회(`clan_join_requests` + `clans`/`games` 분리 로드).
