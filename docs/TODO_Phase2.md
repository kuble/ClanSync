...
|------|-----|
| **단계** | Phase 2 — 앱 구현 |
| **마지막 갱신** | 2026-04-22 — **M3 S02 온보딩 착지** (`0003` `clan_join_requests` · D-AUTH-01 미들웨어 · `/games/[g]/auth`·`/clan` UI · 개발용 게임 연동 시뮬레이터) |
...
| — | `middleware.ts` (전역) | _(해당 없음)_ | M1→M3 | 세션 refresh + D-SHELL-02 + **비로그인 `/games` 차단** · D-LANDING-04 · **D-AUTH-01** 게임 하위 경로 |
...
| M2~M3 UI | `src/app/page.tsx` · `sign-in/` · `sign-up/` · `games/` · `games/[gameSlug]/{auth,clan,page.tsx}` | MainClan `clan/[id]`는 M4 |
