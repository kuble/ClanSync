...
> **지금 라이브**: M7 LFG `0042` · MainGame 게시판 **단일 글 상세** `/board/[postId]` · `?tab=` · 프로필 부계동 `0043` · **다음**: **M8** 감사·Phase 2+ (게시판 댓글·스크림 채팅 등)
...
| **마지막 갱신** | 2026-05-12 — M7 게시판 상세 라우트 `/board/[postId]` · M5 부계동 `0043` |

---

## 다음 세션 권장 프롬프트 (/todo 갱신 시 덮어씀)

**지금 단계(Phase 2 → M8 감사·Phase 2+)** — 복사용:

```
@docs/TODO_Phase2.md 참고해서
**M8 종료 감사** 또는 Phase 2+ (게시판 댓글·스크림 채팅·승부예측 정산 등) 중 한 묶음(한 문장).
