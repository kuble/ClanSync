...
**D-SESSION-01(2026-09-16) 확정 요구:** 세션은 당일 내전 전체이고 경기 한 판은 라운드다([용어 기준](./glossary.md#내전-운영-용어)). 아래는 도메인 관계 요구이며 신규 테이블·컬럼·마이그레이션 적용 완료를 뜻하지 않는다.

- 한 내전 세션에 여러 라운드가 속한다. 세션의 개설·종료 시각, 개설일에서 고정한 내전 날짜, 라운드의 소속 세션·회차를 구분해야 한다.
- 기존 `matches`·출전자·결과와 세션의 연결을 실제 마이그레이션·타입과 대조한 뒤 설계한다. 동일 날짜만으로 라운드를 같은 세션에 묶지 않는다.
- 세션 접근 권한을 가진 클랜원과 각 라운드 출전자를 구분한다. 베팅 자격·결과·정산은 해당 라운드 기준으로 판단한다.
- 세션 종료 시점까지 진행한 라운드를 기록하되 미완료 결과를 자동 생성하지 않는다. 종료 중 라운드 처리·재경기 회차·사후 정정은 후속 결정이다.

### matches (경기 기록 · 내전에서는 라운드)
| 컬럼 | 타입 | 설명 |
|------|------|------|
| id | uuid PK | |
| clan_id | uuid FK → clans | |
...
| played_at | timestamptz | 라운드의 경기 시각(절대시각). **내전 날짜는 이 값이 아니라 소속 세션의 개설일로 고정한다(D-SESSION-01).** 밸런스 화면·내전 기록은 세션의 날짜와 라운드 회차를 사용한다. 통계 등 별도 집계일이 필요하면 해당 집계 정책을 따르되 세션의 내전 날짜를 덮어쓰지 않는다. 기존 KST 06:00 기준을 내전 날짜에 적용하던 설계는 대체한다. |
