...
## 2026-05-11 — 클랜 가입 카드 · 신청 대기 표시

**한 줄 요약**: 신청 후(또는 이미 보류 중인) 클랜 카드에 「신청 대기 중」이 보이고, 같은 카드에서 `가입 신청` 버튼이 반복 활성화되지 않는지 확인한다.

**환경**: `http://127.0.0.1:3000` (`npm run dev`) 또는 Preview URL · 동일 브랜치.

**사전 조건**: [debug-and-fixtures.md](./01-plan/debug-and-fixtures.md) — 게임 연동 완료·클랜 미소속 계정 · 시드 또는 목록에 클랜 카드 존재.

**절차**:

1. `/games/overwatch/clan` 로 이동한다.
2. 아무 클랜 카드에서 **가입 신청 → 보내기**로 신청한다(토스트 또는 상단 안내 확인).
3. 목록이 갱신되면 해당 클랜 카드에 **신청 대기 중** 배지가 보이고 `가입 신청` 버튼이 나타나지 않는지 확인한다.
4. 상단 **가입 신청 대기 중** 배너의 **신청 취소**로 되돌릴 수 있는지 확인한다(선택).

**기대 결과**:

- [ ] 신청 완료 후 해당 카드만 대기 상태로 바뀌고, 같은 카드에서 다시 폼을 열 수 없다.
- [ ] 다른 클랜 카드에서는 기존대로 신청 또는 「다른 클랜 신청 교체」 흐름이 가능하다(D-CLAN-02).

**깨졌을 때**: `clan_join_requests`에 본인 `pending` 행 존재 여부·`/games/[g]/clan` 서버 렌더 `pendingRow` 로드. Network(Server Actions)·브라우저 콘솔.
