...
2. 화면 하단이 아니라 **위쪽** 알림 영역에 「가입 신청을 보냈습니다.」가 잠시 뜨는지 본다.
3. 클랜장(또는 승인 권한 역할)으로 로그인 → 해당 클랜 **관리** → 가입 신청 목록.
4. **거절**만 누르면 사유 입력란이 펼쳐진다. **거절 확정**을 눌러야 거절이 처리된다.
5. **승인**을 누르면 곧바로 처리되는지, 성공 시 안내 토스트가 보이는지 본다.

**기대 결과**:

- [ ] 신청 후 성공 토스트가 보인다(약 0.4초 뒤 목록이 새로고침될 수 있음).
- [ ] 승인 또는 거절 확정 후 성공 토스트가 보이거나, 실패 시 빨간 토스트로 이유가 보인다.

**깨졌을 때**: 브라우저 콘솔·Network(Server Actions), `.env.local`의 `SUPABASE_SERVICE_ROLE_KEY`·Supabase 로그.
