...
### 게시글 상세 (Phase 2 · 읽기 MVP)

**경로**: `/games/[gameSlug]/board/[postId]`

**한 줄**: 홍보·스크림 타입 글 하나를 허브 바깥 **전체 화면**으로 읽습니다 (로그인 + 해당 게임 인증 필요, `middleware` 와 허브 동일).

**영역**

- 제목, 본문(`content`), 작성자 닉네임(`created_by`), 게시 시각, **클랜으로 이동** 링크.
- 타입 라벨(홍보 / 스크림), `is_pinned` 시 「상단 고정」표시.
- 상단 「← 게임 허브 · 홍보 탭」으로 `?tab=promo` 복귀.

**Phase 2+ 후보**

- 홍보 **드로어** 수준의 부가 블록(규칙 요약 등), 댓글·좋아요, 조회 집계.
...
- **목록**: 해당 게임에 소속된 클랜이 `scrim_rooms`의 호스트(`clan_a_id`) 또는 게스트(`clan_b_id`)로 참여 중인 방을 시간순으로 표시한다. 각 행에 제목·일시·장소(선택)·모드·티어 범위·메모(선택)·상대 클랜·`draft`/`matched`/… 상태·호스트/게스트 확정 여부를 보여 준다.
- **개설**: 클랜에 소속되고 `confirm_scrim` 권한이 있는 멤버만 **제목(선택)·일시(필수)·장소(선택)·모드(선택·`varchar(32)`)·티어 하한/상한(선택·함께 입력·0~5000 정수)·메모(선택)** 로 `draft` 방을 생성한다. (INSERT는 서비스 롤 — `createDraftScrimRoomAction`.)
...
- **필터(앱 바)**: `data-testid="scrim-filter-bar"`. **상태 단일 선택** — `전체` / `모집·조율 중`(=`draft`|`matched`) / `일정 확정·종료`(=`confirmed`|`finished`). **티어 다중(OR)** — 프리셋 SR 구간(~800–2300, ~2000–3400, ~3000–5000)과 호스트 `tier_min~tier_max`(둘 다 있을 때)가 숫자 범위로 **한 번이라도 겹치면** 표시 · 티어 미표기 방은 티어 칩과 무관하게 **항상 표시**. **취소 행 기본 숨김**(체크 시 표시).
- **미구현(Phase 2+ 등)**: 스크림 전용 **채팅 모달**.

### 필터
| 항목 | 옵션 |
|------|------|
| 상태 | 전체 · 모집·조율 중 · 일정 확정·종료 (앱 상태 칩) |
| 티어 (다중) | 구간 무시(디폴트) · 저·중티어 묶음 · 중위 묶음 · 고티어 묶음(`OR`; 미표기 방은 항상 통과) |
| 취소 표시 | 기본 숨김 · 필요 시 「취소된 방도 표시」(체크) |
