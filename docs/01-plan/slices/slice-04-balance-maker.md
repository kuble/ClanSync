...
- 수동 명단은 좌우 5칸, 각 팀 **딜·딜·탱·힐·힐** 고정과 중앙 역할 아이콘을 사용한다. 관전자 등록은 제외한다. 이름 클릭 → 고정 순서 첫 빈칸 입력, 자리 우클릭 → 비우기, 위 명단 드래그 → 빈칸 이동/두 사람 교환(2026-09-16 사용자 요청). 별도 자리 선택·교환 모드·반복 위치 문구·체크·색상 범례는 제거한다. 선택자 숨김과 목록 압축의 영향·보조 조작은 `D-UX-01`에서 검토한다.
