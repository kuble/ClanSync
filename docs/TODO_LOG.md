...
- 새 클랜 기본 3시간 자동 종료와 운영진용 `클랜 관리 > 내전 관리 설정` 탭을 추가했다. 세션 편집·역할 선호·맵/영웅 투표·예측을 활동으로 추적하고 5분 주기 DB 작업이 결과 미정 라운드를 무효 종료한다. 깜짝 내전은 기존 종료 삭제 규칙을 따른다.
- QA 66개 적용·타입 동기화. 별도 임시 클랜의 설정 권한, 비활성 유지, 활동 갱신, 만료 종료 DB 테스트와 변경 파일 린트·타입 검사를 통과했다. 운영 DB·전체 회귀·사용자 QA 명단은 변경하지 않았다.
