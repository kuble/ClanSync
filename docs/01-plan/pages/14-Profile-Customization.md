...
- **Phase 2 구현**: 라우트 `dynamic = "force-dynamic"`; 가입 신청·취소·승인·거절 후 `revalidatePath("/profile")`. 본인 목록은 **`fetchMyClanJoinRequests`**(내부 `select_my_clan_join_requests` RPC·`0035_*`)으로 조회한다.
