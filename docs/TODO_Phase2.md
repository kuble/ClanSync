...
**추가 수정 — 세션 자동 종료:** 새 클랜은 마지막 입력 후 3시간 자동 종료를 기본 사용한다. 운영진은 클랜 관리의 내전 관리 설정에서 기능을 끄거나 1~168시간으로 변경한다. 세션 편집·역할 선호·맵/영웅 투표·예측 입력이 활동 시각을 갱신하며 QA cron이 5분마다 만료 세션을 무효 종료한다. QA 66개·운영 50개(16개 대기). 별도 임시 클랜으로 권한·비활성 유지·활동 갱신·만료 종료를 검증했고 변경 파일 린트와 타입 검사를 통과했다. 사용자 QA 명단·전체 회귀·운영 DB는 건드리지 않았다.
