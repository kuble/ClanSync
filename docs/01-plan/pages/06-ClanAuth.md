...
**실구현(Phase 2)**: 카드 내 인라인 신청 폼에서 보내기 시 처리 중 로딩 토스트 후 성공·중복·실패 안내가 화면 **상단 중앙**(sonner)에 표시되며, 목록 새로고침 전에 충분히 읽을 수 있도록 표시 시간을 둔다. 서버에는 게임당 **한 건의 `pending` 신청**만 허용(부분 유니크)·다른 카드 신청 시 기존 신청 교체 확인 다이얼로그가 뜬다. **카드 상태**: 해당 게임에 대기 중인 신청 대상 클랜과 일치하는 카드에는 「신청 대기 중」 배지가 표시되고 `가입 신청` 버튼·폼은 숨겨져 같은 클랜에 대한 중복 신청 UI 오해를 줄인다(상단 배너에서 메시지·신청 취소 가능).
