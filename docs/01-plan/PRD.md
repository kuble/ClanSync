...
- **미결 정책·규칙**: [decisions.md](./decisions.md)에서 추적한다. [BACKLOG.md](./BACKLOG.md)는 후속 구현 메모·에셋 작업에 사용한다.
- **고도화 논의 (2026-09-16~)**: 사용자 요청에 따라 [기획·디자인 고도화안](./product-design-evolution.md)을 구체화한다. `D-UX-01~07`은 논의 중이며, 확정 시 기존 결정과 관련 명세를 함께 갱신한다. 제안 등록만으로 동결된 정책을 변경하지 않는다.
...
- **구성원**: 열린 내전 세션에 사이트로 접근해 실시간 참여, 기능별 권한에 따른 조작·승부예측(Premium), 클랜코인 활동
...
- **밸런스**: 세션은 당일 내전 전체, 라운드는 경기 한 판. 내전 날짜는 세션 개설일로 고정하며 세션 종료까지 라운드·회차를 기록한다. 열린 세션에 클랜원이 실시간 참여하고, 각 라운드는 밸런스장 배치 → 밴픽 → 라운드 시작 → 결과 기록으로 진행한다. Premium은 자동밸런스·OCR·밴픽·승부예측 등. [용어 기준](./glossary.md#내전-운영-용어) · [상세 화면](./pages/09-BalanceMaker.md)
- **통계**: 명예의 전당·내전 통계·개인 기록 3영역. 구성원 공개 범위와 상세 열람 권한을 구분하고, 사이트 방문 지표는 관리의 운영진 전용 영역으로 분리한다. D-STATS-05 기획 확정·구현 대기이며 신규 Premium 제한은 추가하지 않는다([상세 명세](./pages/10-Clan-Stats.md)).
