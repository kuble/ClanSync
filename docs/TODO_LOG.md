...
- 카드 점수·보조 정보 표시 여부와 세션 전적/연승·연패 선택을 라운드 설정에 추가했다. 기본 편성 화면의 중복 승률 막대는 제거하고 팀 합계 정보창에 승률 첫 행, 평가·분석 합계 둘째 행을 배치했다. 팀 제목은 `1팀(+1.7점)` 형식으로 줄였다.
- 무승부 입력·무보상 확정·다음 라운드와 무효 구분 집계를 추가했다. QA 69개 적용, 운영 50개 유지. 관련 DB 7건, 집계/점수 규칙 12건, PC·모바일·호버·키보드·권한·무승부 입력 E2E 2건, 린트·타입·빌드 통과. 정보창 접근성 연결과 소수 표시를 보완하고 임시 픽스처 정리 순서를 수정했다. 사용자 QA 명단은 유지했다.
