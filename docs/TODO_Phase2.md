...
| **마지막 갱신** | 2026-09-18 — 평가/분석 점수·참가자 전적·무승부 입력 |

## 2026-09-16 내전 로비·예약

**추가 수정 — 참가자 점수·전적:** M/A를 평가/분석 점수로 표시하고 개인 점수 배지와 팀 합계를 분리했다. 호버·키보드 정보창에는 현재 세션의 승·무·패/승률/연속 기록과 권한에 맞는 점수를 제공한다. 마이크 저장 정보는 없어 미설정이다. 결과에 실제 무승부를 추가했으며 무효와 분리 집계하고 보상 없이 다음 라운드로 진행한다. QA 68개·운영 50개(18개 대기). 상세 규칙은 [편성 명세](02-design/formation-modes.md#참가자-정보무승부-2026-09-18)를 참조한다.
