...
export function ClanFormationStats({ summaries }: { summaries: IntraStats["scoreGapSummary"] }) {
  ...
  return <Card size="sm">
    ...
    <CardContent className="space-y-4">
      <div className="flex items-center justify-between gap-3"><RubberSegment label="편성 통계 점수 종류" options={[{ id: "evaluation", label: "평가 점수" }, { id: "analysis", label: "분석 점수" }]} value={score} onChange={setScore} /><span className="text-xs text-muted-foreground">{summary.count}경기</span></div>
