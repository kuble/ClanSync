...
**실구현(Phase 2)**: 페이지 **헤더(제목 아래)** 에 `진행 중인 가입 신청`(클랜명·게임명·내용)·**신청 취소**가 보인다. 카드 내 인라인 신청 폼에서 보내기 시 처리 중 로딩 토스트 후 성공·중복·실패 안내가 화면 **상단 중앙**(sonner)에 표시되며, 목록 새로고침 전에 충분히 읽을 수 있도록 표시 시간을 둔다. 서버에는 게임당 **한 건의 `pending` 신청**만 허용(부분 유니크). **카드 상태**: 신청 완료(대기 중) 클랜 카드는 「신청 대기 중」 배지만 보이고 `가입 신청`은 두지 않는다. **같은 게임에 다른 클랜이 이미 대기 중일 때**, 나머지 클랜 카드에서는 일반 `가입 신청` 대신 「이 클랜으로 신청 바꾸기」만 제공해 기존 대기 신청은 취소 후 새로 INSERT 하는 확인 다이얼로그로 진입한다(D-CLAN-02). **조회 패턴**: 신청 상태를 클랜명과 묶을 때 PostgREST `clans!inner` 임베드만 쓰지 않고 **`clan_join_requests` 단독 SELECT 후 `clans`를 별도 조회**해 행 소실을 막는다(`loadGameOnboarding`·`/games` 카드·`submitClanJoinRequestAction`).
