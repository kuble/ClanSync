...
- 수동 명단은 왼쪽 1팀 5칸·오른쪽 2팀 5칸·하단 관전자 구조를 유지한다. 이름 클릭 → 1팀 1~5번·2팀 1~5번 순서의 첫 빈칸 입력, 자리 우클릭 → 비우기(2026-09-16 사용자 요구). 자리 선택·맞교환·반복 위치 문구를 제거하고 하단 선택 여부를 체크·배경으로 구분한다. 원본 자리와 최종 팀·역할 편성을 구분하며 보조 입력은 `D-UX-01`에서 검토한다.
