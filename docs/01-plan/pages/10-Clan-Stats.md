...
| **경기 기록** | 실제 어떤 경기가 진행됐는가 | 독립 상단 탭. 기간·날짜·맵·참가자 조건과 주간 달력 |
...
| 경기 기록 | 기간·맵·참가자 필터 → 주간 달력 → 경기별 팀 명단 | 별도 `view_match_records` 권한 유지 |
...
필터 제목은 옵션 위에 놓고 **부문 / 기간 / 연도 / 월 / 일자**로 짧게 표기한다. 부문·기간·역할은 탄성 세그먼트를 유지하고 연도·월·일자는 세로 휠(마우스 스크롤·터치·클릭·방향키·Home/End)로 선택한다. HoF와 내전 통계는 전체/연도/월까지만 선택한다. 내전 통계의 월별 그래프는 해당 달 전체의 일별 추이를 표시하며 별도 일자 필터는 제공하지 않는다. 경기 기록은 월 안의 특정 일자까지 지원한다. 좁은 화면은 필터를 줄바꿈하고 긴 세그먼트 내부만 가로 스크롤한다.
...
| 경기 아카이브 | 중복된 최근 경기 카드는 제거. 작은 주간 달력에서 날짜 선택, 데스크톱 팀별 명단은 두 줄로 표시. 승리 팀 이름 위에 왕관 표시, 선수별 점수와 맵 위의 `내전` 문구는 생략 | 경기 상세에서 양 팀 명단·역할 확인. 참가자 검색은 경기 기록 제목 옆에 배치하고 날짜·맵 조건과 함께 사용 |
