...
**실구현(Phase 2)**: 페이지 **헤더(제목 아래)** 에 `진행 중인 가입 신청`(클랜명·게임명·내용)·**신청 취소**가 보인다 (개발 빌드에서는 제목 바로 아래 **Dev 반영 확인** 한 줄이 있어 `npm run dev` vs `next start` 번들 차이를 안내한다). **「신청 진행 중인 클랜」 블록**(가입 탭 본문 상단): 페이지네이션·검색으로 목록에 해당 클랜이 없어도 **신청 중인 클랜 카드**가 항상 먼저 보이게 분리한다. 카드 내 인라인 신청 폼에서 보내기 시 처리 중 로딩 토스트 후 성공·중복·실패 안내가 화면 **상단 중앙**(sonner)에 표시되며, 목록 새로고침 전에 충분히 읽을 수 있도록 표시 시간을 둔다. 서버에는 게임당 **한 건의 `pending` 신청**만 허용(부분 유니크). **카드 상태**: 신청 완료(대기 중) 클랜 카드는 「신청 대기 중」 배지와 함께 **`가입 신청됨 · 대기 중` 비활성 버튼**을 둔다(일반 `가입 신청`은 제공하지 않음). **같은 게임에 다른 클랜이 이미 대기 중일 때**, 나머지 클랜 카드에서는 일반 `가입 신청` 대신 「이 클랜으로 신청 바꾸기」만 제공해 기존 대기 신청은 취소 후 새로 INSERT 하는 확인 다이얼로그로 진입한다(D-CLAN-02). **`/profile`**: 가입 신청 제출 등 돌변 후 즉시 보려면 `force-dynamic` + 서버 액션 `revalidatePath("/profile")` 로 캐시를 무효화한다(D-PROFILE-02). **조회 패턴**: 신청 상태를 클랜명과 묶을 때 PostgREST `clans!inner` 임베드만 쓰지 않고 **`clan_join_requests` 단독 조회**(가능하면 **`select_my_clan_join_requests()` RPC** 로 본인 행 고정)·이후 `clans`/`games` 를 별도 조회해 행 소실을 막는다(`loadGameOnboarding`·`/games` 카드·`submitClanJoinRequestAction`·`/profile`).
...
- **Phase 2 (Next.js)** — 가입 탭 클랜 카드: 「가입 신청」 클릭 시 **카드 하단 인라인 패널**(메시지·보내기·취소)을 연다(모달 대신 — 일부 환경에서 Dialog 가시성 이슈 회피). **이미 다른 클랜에 pending**인 경우에만 확인 **모달**을 띄운다. **보내기** 성공 시 상단 **토스트**로 「가입 신청을 보냈습니다.」를 짧게 보여 준 뒤 페이지를 새로고침한다(토스트가 보이도록 새로고침은 아주 짧게 지연).
