  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <Card size="sm" className="relative isolate"><CardHeader><CardDescription>승률</CardDescription><CardTitle className="text-2xl tabular-nums">{rate(totals.rate)}</CardTitle></CardHeader><CardContent className="text-xs text-muted-foreground"><StatsGauge value={totals.wins} total={totals.matches} label={`승률 ${rate(totals.rate)} · ${totals.wins}승 / ${totals.matches}경기`} /><span>승 ÷ (승 + 무 + 패)</span></CardContent></Card>
      </div>
