...
**[D-STATS-05](../decisions.md#d-stats-05) (2026-09-24), 기획 확정·구현 대기:** 앱/사이트 방문 통계를 통계 화면에서 이 관리 콘솔의 운영진 전용 영역으로 이동한다. 아래 목표 구성을 실제 화면·DB 권한에 적용했다는 뜻은 아니다.
...
관리 뷰 진입 (개요 탭 기본)
    │
    ├─ 개요 ──► 클랜 프로필 요약 / 배너·아이콘 편집 / 공지 게시글 / 규칙 / 사이트 이용 통계(구현 대기)
    │
...
### 운영진 편성 통계 (2026-09-24 반영)

개요에서 편성 당시 두 팀 점수 합계 차이의 평균과 구간별 경기 비율을 표시한다. 평가/분석 점수를 전환하며 점수 스냅샷이 없는 경기는 제외한다. 운영진과 `view_mscore` 권한을 함께 확인하고 일반 통계 응답에는 이 집계를 전달하지 않는다. 편성 방식 빈도는 화면에 표시하지 않고 원본·집계만 보존한다.

### 내전 운영 통계 (2026-09-25 반영)

`맵 투표와 선정` 및 `경매 기록` 집계를 공개 내전 통계에서 관리 개요로 옮긴다. 맵별 후보·득표·선정 경기 수와 경매의 역할별 낙찰가·가격 구간·팀 지출/잔액·전략 아이템 구매를 운영진이 확인한다. 일반 멤버는 관리 화면에 접근할 수 없다.

### 사이트 이용 통계 (D-STATS-05 · 구현 대기)

- 개요의 운영진 전용 영역에서 월별·연도별 **방문 활동일(person-day)**과 기간별 **사이트 방문자 수(distinct)**를 조회한다. 권한 없는 member에게는 화면·집계 응답을 제공하지 않는다.
- D-STATS-03의 자기 클랜 첫 페이지뷰·클랜 시간대 자정·하루 1회·봇/프리페치 제외 기준을 유지한다. 실제 페이지뷰 횟수나 플레이 시간을 뜻하지 않는다.
- 방문자 수와 내전 출전 인원은 별개다. 내전 경기 수·순참여인원·참여율은 [내전 통계](./10-Clan-Stats.md)에 남기며, 사이트 미접속을 내전 불참으로 계산하지 않는다.
- CSV는 기존 `export_csv` 권한과 구현 보류 상태를 유지한다. 신규 Premium 제한은 추가하지 않는다.

<a id="site-usage-ui"></a>

**표현·탐색 방식 — [D-STATS-06](../decisions.md#d-stats-06), UI/UX 승인·구현 대기:** 개요 안의 기존 배치를 유지하며 방문 규모 → 변화 → 상세 순서로 읽히게 한다. 통계 화면과 공통인 필터·상세 탐색·접근성 원칙은 [통계 UI/UX](./10-Clan-Stats.md)를 따른다.

| 항목 | 기본 표현 | 인터랙션 |
|---|---|---|
| 방문한 멤버 수 | 기간 내 방문자 수를 핵심 숫자로 표시하고 이전 동일 길이 기간과의 차이를 함께 표기 | 일·주·월 단위 전환, 비교 기간 명시 |
| 방문 활동일 합계 | 방문자 수 아래의 보조 수치로 표시 | `멤버 1명의 하루 방문 = 1활동일`이라는 계산 설명 확인 |
| 방문 추이 | 날짜별 대표 그래프 하나로 추이를 표시 | 기간 선택·이전 기간 비교, 날짜 선택으로 해당 날짜 수치 확인 |
| 월·연도별 비교 | 월별 막대를 기본으로 제공하고 상세에서 연도 × 월 표로 전환 | 연도 선택·그래프/표 전환. CSV는 구현 시 기존 `export_csv` 권한을 적용하며 현재 구현 보류 |

- 기간 내 방문자는 distinct로 계산한다. 일별 방문자 수를 합산한 값을 기간 방문자로 표시하지 않는다.
- 비교할 기록이 없으면 증감을 만들지 않고 `비교 기록 없음`으로 표시한다. 그래프의 색만으로 증감을 전달하지 않는다.
...
1. **가입 요청 카드**
   - 표: 닉네임 / 이메일 / 신청일 / [승인][거절]
   - **승인**: 한 번의 클릭으로 처리. 클릭 직후 처리 중 로딩 토스트가 보이고, 성공·실패는 화면 **상단 중앙** sonner로 안내한다(표시 시간 충분·헤더보다 위 z-index).
   - **거절**: 첫 클릭은 사유 입력(선택) 영역을 펼침. **거절 확정**을 눌러야 DB 반영. 취소로 접을 수 있음. 확정 시에도 로딩 후 결과 토스트를 동일 규칙으로 표시한다.
   - 승인·거절 처리는 가입 신청 상태 머신과 연결 (D-CLAN-02). 서버 액션은 서비스 롤로 갱신하며, 로컬 개발 시 `.env.local`에 `SUPABASE_SERVICE_ROLE_KEY`가 없으면 처리 실패할 수 있음(토스트로 안내).
...
기존 목업 `MOCK_MANAGE_MEMBERS`는 아래 구성을 사용한다. D-STATS-05 적용 시 기록·맵·역할·시너지는 [개인 기록](./10-Clan-Stats.md)과 같은 집계·권한을 참조하고, 관리의 운영 액션은 유지한다. 모달의 이전 예시가 별도 통계 정의나 KDA·영웅 데이터의 구현 완료를 뜻하지 않는다.
...
- D-STATS-03의 `clan_daily_member_activity` 설계에서 활동일 합·기간 내 방문자 distinct를 산출한다. 내전 로스터를 이 방문자 수의 출처로 사용하지 않는다.
- [스키마 명세](../schema.md#clan_daily_member_activity-클랜-활동일--d-stats-03)의 관리 전용 열람 목표와 실제 마이그레이션/RLS 적용 상태는 구현 단계에서 대조한다.
