...
승률·최다 출석·최다 출전·예측 적중 중 한 부문을 선택해 순위 목록과 해당 기간의 누적 순위 변동 그래프를 함께 표시한다. 월별은 기록이 있는 날짜, 연도별·전체는 월별 마감 시점의 누적 순위를 연결한다. 현재 상위 10명을 그래프에 표시하며 등재 기준 미달 또는 12위 밖인 지점은 선을 비운다. 멤버에게는 공개된 부문·순위만 표시하며 비공개 부문이나 등재자가 없는 자리를 샘플 수상자로 채우지 않는다.
