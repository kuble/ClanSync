...
## 2026-05-12 — MainGame 게시판 글 상세(`/board/[postId]`)

**한 줄 요약**: 게임 허브 홍보 탭 또는 홈 「최근 홍보」에서 제목으로 **단일 글 상세**로 들어가고, 본문·클랜 링크·뒤로가기가 정상인지 확인한다 (`board_posts`, 로그인+게임 인증 가동일).

**환경**: `http://127.0.0.1:3000`(또는 Preview) · [debug-and-fixtures.md](./01-plan/debug-and-fixtures.md).

**사전 조건**: 해당 게임 **계정 연동 완료** 계정 · 화면에 보이는 `board_posts` 행 하나(없으면 홍보 탭에서 먼저 등록).

**절차**:

1. 테스트 계정 로그인 후 `/games/overwatch`(또는 대상 게임 슬루그) 접속 · **탭 또는 홈**에서 홍보 제목 링크 클릭.
2. 주소가 `/games/{slug}/board/{uuid}` 인지 확인.
3. `data-board-post-detail="{slug}"` 영역에서 제목·본문·「클랜」링크 확인.
4. 「← … 홍보 탭으로」로 돌아가 `?tab=promo` 가 열리는지 확인.
5. URL의 `uuid`를 임의 문자열로 바꿔 접속했을 때 404 페이지가 되는지 확인(선택).

**기대 결과**:

- [ ] 미로그인·미인증 시 미들웨어에 따라 더 이상 진행되지 않는다.
- [ ] 다른 게임의 글 UUID로 슬루프만 바꿔 접근하면 해당 글이 없으면 404다.
- [ ] `promotion` / `post_type`(스크림 글 존재 시) 배지 카피만 다르게 보인다.

**깨졌을 때**: RLS(`board_posts_select_auth`) · `middleware` 게임 인증 분기 · Network `404`/`PGRST116`.

## 2026-05-12 — 프로필 게임별 탭 부계동(자기신고)·동의 후 등록(M5 후속)

**한 줄 요약**: 게임을 **인증 완료**한 탭에서만 부계동을 추가할 수 있고, 공개 안내·동의 체크 후 등록·삭제가 되는지 확인한다 (`user_alt_accounts` · `0043_user_alt_accounts.sql`).

**환경**: `http://127.0.0.1:3000`(또는 Preview) · [debug-and-fixtures.md](./01-plan/debug-and-fixtures.md).

**사전 조건**: `user_game_profiles.is_verified = true` 인 테스트 계정(시드 또는 인증 플로 완료). 마이그레이션 **`0043`** 적용 및 `npm run types:gen` 반영됨.

**절차**:

1. 테스트 계정 로그인 후 `/profile` 이동.
2. 게임 탭 중 **연동 인증 완료**된 게임을 연다 · 하단 「부계동 (자기신고)」블록과 `data-alt-accounts-panel="(게임슬루그)"` 가 보이는지 확인(미인증 게임 탭에는 블록 없음).
3. 소속 클랜이 있으면 블린트별 공개 설정 요약 줄이 나오고, 없으면 "본인만 조회" 안내인지 확인.
4. 「+ 부계동 추가」→ 공개 고지 확인 → 닉네임 입력 → 동의 체크 → 「추가」→ 목록과 `data-alt-account-row`(해당 게임 슬루그) 에 행이 생기는지 확인.
5. 「삭제」→ 확인 후 목록에서 사라지는지 확인.

**기대 결과**:

- [ ] 인증 미완료 게임 탭에는 부계동 블록이 없다.
- [ ] 동의 없이 「추가」는 불가하고, 저장 후 새로고침해도 행이 유지된다.
- [ ] 중복 닉(같은 게임 내) 저장 시 사용자에게 에러 안내가 있다.

**깨졌을 때**: Network `user_alt_accounts` REST · RLS 정책(본인 INSERT·본인 삭제)·`effective_view_alt_account_roles` · `403` 또는 duplicate key 메시지.
...
**환경**: `http://127.0.0.1:3000` · 픽스처 **premium** `QA_01_Clan`.

**사전 조건**: [debug-and-fixtures.md](./01-plan/debug-and-fixtures.md) — `QA_Leader_01` 로그인 · MainClan 밸런스 탭.

**절차**:

1. `/games/overwatch/clan/{UUID}/balance`.
2. `data-testid="clan-balance-session-panel"` 이 `data-balance-phase="none"` 인지 확인.
3. 「맵 밴 사용」**체크 해제** → **세션 열기**.
4. **맵 밴 없이 경기 화면으로** → 「③ 경기 진행」배너가 보이는지 본다(맵 라벨 없이 진입한 경우 카피로 안내).
5. **세션 종료** → 패널이 다시 `data-balance-phase="none"`·「세션 열기」로 돌아오는지 확인.

**기대 결과**:

- [ ] 운영진만 세션을 열고·단계 진행·종료할 수 있다.
- [ ] 시드 재실행 시 픽스처 클랜에 **남아 있던 미종료 밸런스 세션**이 정리된다.

**깨졌을 때**: `balance_sessions_one_open_per_clan` 유니크 · RLS(officer)·`ClanBalanceSessionRealtime` 채널.
