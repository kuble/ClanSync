      ...
      <div className="mx-auto max-w-lg space-y-4">
        ...
        {controlledTeams.length ? <div className="space-y-3 pt-2">
          <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2">
            <div className="grid gap-1 min-[480px]:grid-cols-2">{[-100, -10].map((delta) => <Button key={delta} size="sm" variant="outline" className="px-2 tabular-nums" disabled={!actionable || clock >= lot.deadline || amount <= minimum} onClick={() => setBidDraft({ lot: lotKey, value: Math.max(minimum, amount + delta) })}>{delta}</Button>)}</div>
            ...
            <div className="grid gap-1 min-[480px]:grid-cols-2">{[100, 10].map((delta) => <Button key={delta} size="sm" variant="outline" className="px-2 tabular-nums" disabled={!actionable || clock >= lot.deadline || amount + delta > 100000} onClick={() => setBidDraft({ lot: lotKey, value: amount + delta })}>+{delta}</Button>)}</div>
          </div>
