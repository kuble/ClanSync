...
> **D-STORE-04** (DECIDED 2026-09-25 · 추후 구현) — 클랜 테마를 스토어에서 구매하고, 보유 테마를 선택하면 클랜 페이지에 즉시 적용한다. [§D-STORE-04](../decisions.md#d-store-04--클랜-테마-구매선택즉시-적용).
...
스토어 진입 (클랜 꾸미기 탭 기본)
    ...
    │     · 클랜 테마 구매 → 보유 테마에서 선택 → 클랜 페이지 즉시 변경 (D-STORE-04, 추후 구현)
...
### 클랜 테마 (D-STORE-04 · 추후 구현)
- 클랜 스토어에서 테마를 미리 보고 클랜 풀로 구매한다. 구매가 완료되면 해당 클랜의 보유 테마에 추가한다. 가격과 카탈로그는 구현 시 정한다.
- 보유 테마 중 하나를 선택하면 그 클랜의 홈과 하위 페이지에 즉시 적용되고, 모든 방문자가 같은 테마를 본다. 다른 보유 테마를 선택하거나 기본 테마로 되돌릴 수 있다.
- 구매와 적용은 별도 행동이다. 구매만으로 현재 선택된 테마를 바꾸지 않는다. 테마 선택에는 코인을 다시 차감하지 않는다.
- 적용 범위는 해당 클랜의 페이지다. 랜딩 페이지, 다른 클랜, 개인 프로필의 테마는 바꾸지 않는다.
- 첫 테마 시안은 이번에 확정한 **포털 콘셉트**: 짙은 차콜 바탕의 촘촘한 격자 질감, 베이지색 글자와 얇은 구분선, 절제된 하늘색 포인트, 같은 곡률의 둥근 외곽. 시안 구현은 `c6097ad` 커밋을 참고한다. 현재 웹페이지에는 적용하지 않는다.
- 구매 권한과 클랜 풀 지출 제한은 아래 기존 정책을 따른다. 보유 테마 선택 권한은 클랜 운영진+로 한다.
...
- 개인 꾸미기는 프로필, 일부 클랜 꾸미기는 클랜 관리에서 적용한다.
- **클랜 테마는 예외**: 스토어의 보유 테마에서 선택하면 클랜 페이지에 즉시 적용한다 (D-STORE-04 · 추후 구현).

## 목업과 실제 구현의 차이
- 클랜 테마의 구매·보유·선택·즉시 적용은 확정된 **추후 구현** 항목이다. 현재 웹페이지는 기본 테마를 유지한다.
...
- ~~D-STORE-04 클랜 테마 구매·선택·즉시 적용~~ — **DECIDED 2026-09-25**, 구현 예정. §클랜 테마 참고.
...
- [decisions.md](../decisions.md) (D-STORE-01/02/03/04 · D-ECON-01/02 DECIDED)
