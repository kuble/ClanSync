...
**한 줄 요약**: ClanAuth에서 신청 후 **헤더 + 「신청 진행 중인 클랜」(목록 따로)·비활성화된 가입 버튼**이 보이고, `/profile`에서도 요약·목록이 맞으며, 신청 후 **바로 새로 고침 없이**(또는 한 번 새로 고침) 반영된다. 로컬은 `npm run dev`인지 확인한다.

**환경**: `http://127.0.0.1:3000` (`npm run dev`) 또는 Preview URL · 동일 브랜치. **변경 즉시 보려면 `npm run dev`** — `npm run build` 후 `next start`만 돌리면 코드 바꿀 때마다 다시 빌드해야 한다.

**사전 조건**: [debug-and-fixtures.md](./01-plan/debug-and-fixtures.md) — 게임 연동 완료·클랜 미소속 계정 · 시드 또는 목록에 클랜 카드 2개 이상.

**절차**:

1. `/games/overwatch/clan` 로 이동한다. (개발 모드면 제목 아래 **Dev 반영 확인** 문구가 보이는지 본다.)
...
7. `/profile` 로 이동한다. 헤더에 **Dev 반영 확인** 줄이 있으면 최신 빌드로 본 페이지임을 이용해 문구 교체 테스트를 한다.
8. 「**진행 중인 가입 신청**」 요약과 「가입 신청」 목록에 동일 신청(A)이 표시되는지 본다. 안 보이면 **F5** 한 번 한다.
...
- [ ] 개발 빌드에서 Dev 안내 줄이 확인 가능하다 (`data-testid="dev-clan-auth-build-marker"` 등).

**깨졌을 때**: `clan_join_requests` 본인 `pending`, `/profile` 로그 `[profile] clan_join_requests` 메시지, Network(Server Actions)·RLS·embed 없는 조회(`clan_join_requests` + `clans`/`games` 분리 로드). `next start`만 쓸 때 오래된 화면이면 프로세스 종료 후 `npm run build` 재실행.
