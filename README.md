...
클랜 페이지 응답 시간은 QA 서버 실행 후 `node scripts/measure-qa-pages.mjs`로 측정합니다. 기존 QA 리더로 홈·로비·통계·이벤트·관리·상점을 3회 요청하며 최초 응답·본문 완료 시간과 압축 해제 후 본문 크기를 출력합니다. 시드·브라우저 렌더 측정은 수행하지 않습니다.
...
분포가 다양한 추가 통계는 `node scripts/add-qa-stats-variety.mjs`로 미리 확인하고 `--apply`로 적용합니다. 2021~2026년 종료 내전 195회·973경기(무효 20경기 포함)·선호 맵 투표 8,902개·관전자 예측 1,129개를 `[통계 QA] 연도별 활동·선호 분포`로 추가합니다. 연도/월별 활동량, 출석·출전·승률·예측 정확도, 맵·영웅 선호가 다르며 **2024년 3월**은 규정 경기 미달, **2021년 4월**은 빈 기간을 확인하기 좋습니다. QA 허용 대상과 기존 12명을 검증하고, 고정 ID로 재실행/중단 재개를 지원하며 기존 기록·설정·코인은 덮어쓰지 않습니다. 생성기 검증: `node scripts/qa-stats-variety.test.mjs`. 적용 후 완료 경기/득표 집계는 무효 경기를 제외합니다.
