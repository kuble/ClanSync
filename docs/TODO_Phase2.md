...
|------|-----|
| **단계** | Phase 2 — 앱 구현 |
| **마지막 갱신** | 2026-04-21 — **M2 S01 인증 쉘 착지** (`/`·`/sign-in`·`/sign-up`·`/games` + `0002` 로그인 잠금·게임 시드·`db:seed`·미들웨어 가드) |
...
| 05 | `/games/[gameSlug]/auth` | `game-auth.html` | M3 | stub |
| 06 | `/games/[gameSlug]/clan` | `clan-auth.html` | M3 | stub |
| 07 | `/games/[gameSlug]/clan/[clanId]` | `main-clan.html#dashboard` | M4 | stub |
...
| — | `middleware.ts` (전역) | _(해당 없음)_ | M1→M2 | 세션 refresh + D-SHELL-02 + **비로그인 `/games` 차단** · D-LANDING-04 `/` 리다이렉트 |
...
| M2 UI | `src/app/page.tsx` · `sign-in/` · `sign-up/` · `games/` · `games/[gameSlug]/*` 스텁 | 온보딩 스텁은 M3/M4 착수 시 교체 |
