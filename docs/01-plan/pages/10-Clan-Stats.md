...
엠블럼·맵별 기록·시너지는 각각 최대 256px·288px·320px의 내부 스크롤 영역으로 제한한다. 스크롤바는 숨기고 실제로 더 남은 위/아래 가장자리만 투명하게 처리한다. 짧은 목록이나 끝에 도달한 방향은 흐리지 않으며 휠·터치·키보드 스크롤을 지원한다. 필터는 스크롤 영역 밖에 유지한다.
...
| 승부예측 기록 | 본인의 실제 원장에 따른 일별 수익(+)·손실(-) 그래프와 기간 합계. 적중/실패 목록 제거 | KST 정산 날짜 기준. 현재 규칙은 실패 차감이 없으며 미지급 더미·무승부·무효는 0pt. 다른 클랜·다른 사람의 거래 제외 |
| 수상 이력 | 부문별 아이콘·외곽선과 금은동 색상. 날짜·내용은 아이콘 아래에 중복 표시하지 않음 | 호버·포커스·탭으로 기간·부문·순위 확인 |
