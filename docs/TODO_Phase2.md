...
| **마지막 갱신** | 2026-09-26 — QA 통계 분포·연도 확대 |

## 2026-09-26 내전·개인 기록 화면 피드백

- 내전 역할 아이콘·맵 유형 아이콘/이름, 같은 행의 준비/종료 버튼, 로비 종료 목록 제거를 반영했다. 정규 기록은 보존한다.
- 개인 배너와 대표 엠블럼 3개·컬렉션 팝업, 역할 조합표 아이콘, 상위 10개+기타 맵 출전 도넛과 승률 범례를 적용했다. 선택 멤버의 예측 참여/적중 통계를 복원하고 포인트 원장은 본인만 유지한다.
- 별도 QA 프로덕션 빌드·변경 ESLint·타입 검사, 집중 E2E 3건 통과: 내전 준비 행 정렬/맵·영웅 흐름, 개인 연월/엠블럼 호버·키보드·모바일 폭/도넛, 예측 직원·멤버 조회 경계. 실제 QA 배너·컬렉션·맵 도넛도 확인. 공유 QA 시드·운영 DB 변경 없음.

## 2026-09-26 QA 통계 분포·연도 확대

- 사용자 요청으로 `add-qa-stats-variety.mjs`를 QA에 적용했다. 2021~2026년 종료 내전 195회, 973경기(무효 20), 선호 맵 투표 8,902개, 관전자 예측 1,129개를 추가한다. 기존 균등한 과거 데이터는 유지하며 새로운 기록만 고정 ID로 구분한다. 계정·설정 변경, 코인 정산 RPC, 시드 초기화는 수행하지 않는다.
- 생성기는 기간별 성수기/비활동, 출전·출석 차이, 승률·예측 정확도, 가중 맵 선택과 영웅 밴을 재현한다. 2024년 3월은 최소 4경기 대비 `QA_Member_06` 3경기·`QA_Member_12` 1경기, 2021년 4월은 빈 달이다. 실제 게임 출시 연표 재현용 데이터는 아니다.
- 생성기 검증 4건과 스크립트 린트 통과. 최초 저장값 비교에서 JSON 직렬화가 `-0`을 `0`으로 바꾸는 차이를 발견해 생성기의 0 표기를 정규화했다. 재실행은 누락 내전/미완료 경기 0이며 모든 경기 필드·맵 투표·예측이 생성 결과와 일치한다. 재실행 전후 기존 클랜·내전·명단·설정·코인 스냅샷도 동일하다.
- 실제 QA 화면에서 6개 연도 선택·연도별 추이, 전체 내전 276회·완료 1,260경기, 승률 37.6~57.5%, 맵/선호 득표 차이, 2024년 3월 운영진 미달 목록 2명을 확인했다. 기존 애플리케이션 코드는 변경하지 않았으며 운영 DB·수동 배포는 보류한다.

## 2026-09-26 통계 기간 필터·선호 맵·순위와 반응

- 경기 기록을 독립 상단 탭으로 분리했다. 전체/연도/월/일 필터가 내전 요약·참여 추이·맵·영웅 밴·선호 맵을 함께 바꾸며, 전체→연도/연도→월/월→일 추이와 호버·키보드 수치 조회를 제공한다. 집계는 KST 개최일 기준이며 고유 인원을 기간별로 다시 계산한다.
- 오른쪽에 요약 3개를 통합하고 맵·밴·선호 맵은 도넛과 전체 상세 목록으로 표시한다. 7개 이상은 상위 5개+기타. 영웅 밴은 밴 건수, 선호 맵은 실제 후보 득표가 분모다. 운영 관리의 후보·득표·최종 선정 표는 유지한다.
- 제목 위 배치·부문/기간 명칭·연월일 휠을 적용했다. 큰 순번·상위 3명 강조, 최소 규정 경기 안내와 운영진 전용 미달 목록(미출전자 포함)을 추가했다. 댓글은 말풍선·하단 작성창과 가벼운 대화 문구로 바꿨으며 자체 이모티콘 등록·첨부는 후속 명세로 남겼다.
- QA 프로덕션 빌드·타입 검사·변경 파일 린트, 집계/HoF/댓글/기간 UI 회귀 **22건** 통과. 임시 클랜에서 기간별 지도·값 변경, 휠·방향키·호버, 미달 명단의 멤버 응답 제외, 댓글 저장·분리·페이지 이동·삭제, 390px 넘침을 검증했다. 실제 QA 화면에서 순위·반응·그래프 오른쪽 요약 배치도 확인했다. 공유 QA 시드·운영 DB·수동 배포 없음.

## 2026-09-26 명예의 전당 반응

- 누적 순위 변동 컴포넌트·타입·집계·응답 데이터·전용 테스트를 제거했다. 기존 부문 필터와 현재 순위를 유지하고 오른쪽에 반응 창을 배치한다. 좁은 화면은 위아래로 표시한다.
- 클랜·부문·전체/연도/월별 댓글 저장, 500자 제한, 최신순 30개와 커서 기반 더 보기, 새로고침, 본인/운영진 삭제를 구현했다. 선택된 대화만 조회하고 댓글 변경 시 통계를 재집계하지 않는다.
- `20260926011407_hof_comments.sql`을 QA에 적용하고 생성 타입을 동기화했다. 활성 멤버·공개 부문·기간별 공개 정책과 작성자/작성 시각 변경 금지를 DB에서도 적용한다. 운영 DB 적용은 보류한다.
- 타입·변경 소스 린트, QA 프로덕션 빌드 및 관련 회귀 **19건** 통과. 임시 클랜에서 DB 직접 접근(비로그인/타 클랜/탈퇴/작성자 위조/비공개/삭제 권한), 실제 화면의 저장·새로고침 유지·부문/기간 분리·페이지 경계·멤버/운영진 삭제·390px 표시를 검증했다. 기존 QA 데이터는 보존했다.

## 2026-09-25 클랜 페이지 응답 최적화

- 과거 순위 집계에서 `Intl.DateTimeFormat`을 행마다 생성하던 비용을 제거하고 경기 날짜·현재 연월을 반복 계산하지 않게 했다. 권한 8개는 매번 별도 RPC/설정을 읽지 않고 해당 요청에서 읽은 소속·설정으로 판정한다. 요청 간 인증/권한 캐시는 추가하지 않았다.
- 관리 페이지는 운영 통계만 계산하고 명예의 전당·전원 개인 기록 집계를 생략한다. 개인 기록의 반복 아군/상대 객체는 응답 안에서 재사용해 RSC 중복 직렬화를 줄인다. 상점의 잔액·상품·보유 목록 조회는 병렬화했다.
- 동일 QA 클랜 303경기, `next dev` 3011에서 각 페이지를 3회 요청하고 마지막 2회 본문 수신 완료 시간을 비교했다. 통계 **9.60~9.72초 → 0.70~0.74초**, 관리 **9.95~10.08초 → 0.49~0.56초**, 상점 **0.42~0.98초 → 0.30~0.39초**. 나머지 홈·로비·이벤트는 최종 0.26~0.48초다. 통계의 압축 해제 후 본문은 5002 → 3343 KiB로 줄었다. 최초 컴파일·브라우저 렌더 완료·운영 배포 성능 측정은 아니다.
- 재현: QA 서버 실행 후 `node scripts/measure-qa-pages.mjs`. QA 허용 DB·기존 계정을 사용하며 시드는 실행하지 않는다. 변경 파일 린트·타입 검사, QA 대상 프로덕션 빌드 및 집계/계정 격리/개인 공개/관리 접근/상점 표시 26건을 확인했다. 기존 개인 공개 테스트의 계정 전환 오류를 수정하고 해당 1건은 개발 서버에서 재검증했다. 중단된 재시도의 임시 클랜·계정도 정리했다. 운영 DB·수동 배포는 실행하지 않았다.

## 2026-09-25 명예의 전당 순위 변동·필터

- 공개된 네 부문 중 하나를 골라 현재 순위와 기간 내 누적 순위 변동을 함께 본다. 월별은 기록일, 연도별·전체는 월별 시점을 사용하고 공개 시점·상위 노출 설정을 같은 집계에 적용한다. 선택형 통계 필터는 탄성 세그먼트로 통일하고 긴 월 선택은 연도·월로 나눴다.
- 명예의 전당 집계 회귀 6건, 변경 파일 린트·타입 검사 및 QA 화면에서 부문·월별 전환을 확인했다. QA 데이터와 운영 DB는 변경하지 않았다.

## 2026-09-24 통계 인터랙션·개인 공개 설정

- 설명을 `i` 도움말, 기간·역할·정렬을 Option Wheel로 압축했다. 개인 기록은 기본 운영진 전용이며 `hof_config.member_personal_records`가 true일 때만 멤버 본인 데이터/탭을 제공한다. 설정 저장은 활성 운영진과 `set_hof_rules` 권한을 모두 확인한다.
- 개인 기록의 중복 출전 목록을 제거하고 시너지 명칭·승/무/패 형식을 적용했다. 과거 월/연 상위 3위의 금·은·동 엠블럼을 표시하며 정정에 따른 재계산을 허용한다. 영구 수상 스냅샷은 미구현이다. 관련 회귀 22건·타입·린트·QA 빌드를 검증했다.

## 2026-09-24 통계 화면 피드백 반영

- 명예의 전당의 네 부문과 과거 월/연도 선택, KST 출석 집계 및 기간별 내전 횟수·개최 일수·경기 수를 구현했다. 부문 배치는 위의 최신 화면 구성을 따른다.
- 승률·출석·출전·맵·밴·개인 역할/맵 기록에 비율 게이지, 월 추이에 꺾은선을 적용했다. 편성 점수 차이는 관리 전용 응답/화면으로 옮겼다. 회귀 20건·린트·타입·QA 빌드와 반응형 배치를 검증했다.

## 2026-09-24 클랜 통계 1차 구현

- `/stats`를 명예의 전당·내전 통계·개인 기록으로 재구성하고, 사이트 방문을 운영진 `관리 → 개요`로 옮겼다. 정규 내전의 유효 결과만 사용해 승·무·패, 세션 참여와 경기 출전, 맵·밴·편성·경매·점수 차이·예측을 집계한다. 과거 월·연 순위는 현재 원본으로 재계산하며 영구 스냅샷은 아니다.
- 일반 멤버에게 원본 경기·타인 상세를 내려주지 않으며, 개별 점수는 `view_mscore`에 묶었다. 방문 원본과 확정 후 예측 원본 RLS를 운영진/본인 범위로 조정했다. 진행 중 투표 열람은 유지한다. 개인 비공개 오버라이드의 영속화 전에는 타인 상세·상대별 행을 운영진으로 제한한다.
- QA 마이그레이션 2개를 적용하고 타입을 생성했다. 집계·기간 공개·권한·화면을 격리 E2E로 검증했고 타입·변경 파일 린트·빌드가 통과했다. 사용 중인 QA 명단은 시드로 초기화하지 않았으며 운영 DB·배포는 보류한다. 전체 기획의 남은 항목은 [통계 명세 §7](01-plan/pages/10-Clan-Stats.md)에 기록한다.

## 2026-09-23 보안·기능 검토 수정

- 보안 7건·기능 오류 8건과 경매 증감 단위를 수정했다. 권한을 서버/DB에서 강제하고 투표·정산 경합을 직렬화했으며 기존 멤버 집계 화면과 내전 흐름을 보존했다. 독립 재검토에서 확인한 달력 시간대 회귀도 수정했다.
- QA 마이그레이션 80개 적용·생성 타입 동기화. 집중 회귀 19개, 기존 내전/밴픽/예측 DB 20개, 린트·타입·최종 빌드 통과. 운영 DB·배포 보류. 세부 범위와 검증 한계는 [수정 보고서](03-analysis/review-fixes-2026-09-23.md)에 기록한다.

## 2026-09-22 추가 영웅 밴 투표 검증

- D.Mon 단독 선택으로도 `팀별 밴 개수 이내로 서로 다른 영웅을 선택하세요.` 오류가 재현됐다. DB RPC의 구형 42명 허용 목록을 화면과 같은 53명으로 수정하고 미지원 ID 오류를 개수/중복 오류와 구분했다. 투표 권한·잠금·마감 식별자·기권 처리는 유지한다.
- QA 마이그레이션 78개 적용·생성 타입 동일. 격리 DB 테스트 7건(누락 11명 저장, 연속/동시 갱신, 중복·개수·미지원 거부, 권한·마감 포함), 집계 규칙 3건, 집중 브라우저 1건(더블클릭·취소/재선택·추가 영웅 확정), 린트·타입·빌드 통과. 변경 함수 보안 advisor 경고 없음. 브라우저 테스트는 사라진 네트워크 응답 본문 대신 실제 화면과 저장 결과를 검증하도록 보완했다.
- 동일 팀에서 서로 다른 두 영웅이 1표씩이면 1밴 설정은 영웅 ID 순서상 앞선 1명, 2밴 설정은 둘 다 선정된다. 서로 다른 팀의 투표는 각각 집계한다. 이 규칙은 변경하지 않았다. 운영 DB·사용 중인 QA 명단은 변경하지 않았다.

## 2026-09-18 경기 현황 간소화·승부예측 설정

- 점수 모드 토글은 편성 화면에만 두고 경기 현황 참가자는 이름만 표시한다. 기존 카드 표시 설정과 관계없이 개인 점수·팀 합계·보조 정보·정보 팝업을 숨긴다. 승부예측과 권한이 있는 운영진의 점수 조정은 각각 별도 드로워로 제공한다.
- 라운드 설정의 밴픽 탭에서 Premium 정규 내전만 승부예측 사용 여부를 바꾼다. 기본값은 기존 동작을 유지하는 사용이며 다음 라운드로 이어진다. Free·깜짝 내전은 UI에서 잠기고 예측 제출도 서버·RLS에서 거부한다. 경기 시작 후 변경은 금지하며 관전자·마감 시각 조건은 유지한다.
- 평가·분석 점수는 −10~10의 유한 소수를 허용한다. 잘못된 입력을 저장 시 보정하지 않고 UI·서버에서 거부한다. QA 마이그레이션 76개 적용·생성 타입 확인, 운영 50개 유지.
- 검증: 관련 DB 19건(새 설정 6·밴픽/선호 13), 점수 규칙 5건, 집중 E2E 2건(기존 정보 표시·새 설정/드로워), 변경 파일 린트·타입·빌드 통과. 운영진/멤버 화면, Free 잠금, Premium 저장·해제, 참가자 참여 거부·관전자 예측 변경, 모바일, ±10 저장과 10.1 거절을 확인했다. 테스트 전용 개발 캐시의 404를 정리한 뒤 통과했으며 사용자 QA 서버·명단·운영 DB·전체 회귀는 변경/실행하지 않았다.

## 2026-09-18 주장 지명·자동 경매·전략 아이템

- 지명·경매 영역을 참가자 표 위로 옮겼다. 주장을 겸하는 관리자는 자기 팀만 조작하며 서버도 상대 팀 입력을 거부한다. 경매는 좌우 증감 버튼, 현재 선수·타이머·낙찰 연출과 자동 다음 경매를 제공한다. 연결 중인 참가자 누구나 서버 기한 확인을 요청할 수 있어 관리자의 버튼 클릭이나 화면 유지에 의존하지 않는다.
- 아이템 기능을 켜면 무작위 3개 공개·전략 준비·선수 경매·양 팀 아이템 선택·편성 완료로 진행한다. 준비 시간과 경매 시간을 설정하며, 클랜 운영진은 내전 관리 설정에서 아이템을 관리한다. 기본 구매 규칙은 팀당 최대 1개·양 팀 동일 구매 가능·남은 예산 사용이고 효과는 운영진이 직접 적용한다. 공개한 내용은 경매별로 고정한다. 상세는 [편성 명세](02-design/formation-modes.md#주장-조작자동-경매전략-아이템-2026-09-18)를 따른다.
- 양 팀 동시 선택은 현재 편성 식별자와 최신 상태를 대조해 합치며 중복 차감·초기화 전 요청을 차단한다. 기존 라운드에 없는 새 설정값은 기본값으로 보완한다. QA 마이그레이션 75개 적용·생성 타입 확인, 운영 50개 유지. 사용자 QA 명단은 시드로 초기화하지 않았다.
- 검증: 편성 규칙 22건, 관련 DB 22건(편성 8·아이템 권한 1·밴픽/선호 13), 집중 E2E 3건(카탈로그·주장 경매·기존 직접 배정/공통 추첨), 변경 파일 린트·타입·빌드 통과. 실제 전략/입찰 기한·낙찰 후 다음 선수, 관리자 이탈 후 멤버 진행, 양 팀 동시 구매/기권과 중복 차감 방지, 초기화 전 요청 차단, 공개 아이템 고정, 858/390/320px 화면을 확인했다. 전체 회귀·운영 반영은 수행하지 않았다.

## 2026-09-16 내전 로비·예약

**추가 수정 — 편성 진행 간소화:** 직접 배정은 `다음 단계` 한 번으로 저장·맵 단계 이동을 처리한다. 공통 추첨은 별도 팝업 없이 현재 편성판에서 같은 서버 결과를 순차 공개하고 완료 후 자동 이동한다. 지명·경매도 편성 완료 시 자동 적용한다. 린트·타입과 직접 배정/밴픽 3건, 멤버 권한 1건, 공통 추첨·자동 전환·기록 1건, 편성 규칙 17건을 통과했다. DB 스키마·운영 환경은 변경하지 않았다.

**추가 수정 — 참가자 점수·전적:** M/A를 평가/분석 점수로 표시하고 개인 점수 배지와 팀 합계를 분리했다. 호버·키보드 정보창에는 현재 세션의 승·무·패/승률/연속 기록과 권한에 맞는 점수를 제공한다. 카드 점수·보조 정보·팀 비교 및 플레이어 정보 팝업의 표시 여부와 세션 전적/연승·연패 선택을 라운드 설정에 추가했고, 설정은 편성/화면 표시/밴픽 탭으로 분리했다. 결과에 실제 무승부를 추가했으며 무효와 분리 집계하고 보상 없이 다음 라운드로 진행한다. QA 70개·운영 50개(20개 대기). 상세 규칙은 [편성 명세](02-design/formation-modes.md#참가자-정보무승부-2026-09-18)를 참조한다.

검증: 관련 DB 7건·집계/점수 규칙 10건·집중 E2E 1건, 린트·타입·빌드 통과. PC/모바일, 점수 모드·합계·교환, 호버/Tab/Escape, 일반 멤버 비공개, 실제 무승부 입력 후 1승 2무 1패/25% 갱신을 확인했다.

**추가 수정 — 세션 자동 종료:** 새 클랜은 마지막 입력 후 3시간 자동 종료를 기본 사용한다. 운영진은 클랜 관리의 내전 관리 설정에서 기능을 끄거나 1~168시간으로 변경한다. 세션 편집·역할 선호·맵/영웅 투표·예측 입력이 활동 시각을 갱신하며 QA cron이 1분마다 만료 세션을 무효 종료한다. 기존 세션의 활동 시각이 배포 시각으로 초기화되던 문제를 실제 확인 가능한 마지막 입력 시각으로 보정했다. QA 67개·운영 50개(17개 대기). 별도 임시 클랜의 권한·비활성 유지·활동 갱신·만료 종료 DB 테스트와 실제 기존 정규·깜짝 세션의 예약 자동 종료를 확인했다. 사용자 QA 명단·전체 회귀·운영 DB는 건드리지 않았다.

**추가 수정 — 팀별 영웅 밴:** 팀당 1/2명 설정·초상화 즉시 투표·팀별 득표순(무투표는 없음)·밴 확정과 경기 시작 원자 저장을 적용했다. 역할/팀별 밴 맥락과 모델 입력 키를 준비했으며 실제 숙련도 승률 모델은 후속이다. QA 65개·운영 50개(15개 대기). 린트·타입·빌드·관련 DB 6건·집계 1건·집중 E2E 1건 통과. 새 분석 컬럼 저장 권한을 운영진 RLS 범위로 보완하고 멤버 쓰기 차단을 검증했다.

**추가 수정 — 정규 승부예측 제한:** 액션과 DB의 제한 정책으로 정규 방만 예측을 등록/수정할 수 있다. 기존 Premium·관전자·미종료·마감 조건을 유지한다. 깜짝 UI는 계속 예측 참여를 숨긴다. 팀 합계/총점·토글 옆 안내·카드의 중복 역할 표기를 제거했다. QA 63개 적용·운영 50개(13개 대기), 생성 타입 동일·새 정책 advisor 경고 없음. 린트·타입·빌드와 집중 E2E 1건 통과(정규 예측 등록/수정, 깜짝 신규 예측 거절·기존 예측 변경 불가·정규 예측의 깜짝 이동 거절, UI). 마이그레이션의 중복 정의를 제거한 후 QA 적용에 성공했다. 시드·전체 회귀는 실행하지 않았다.

**추가 수정 — 맵 결과·승률 간소화:** 실제 결과 팝업을 본문 카드 강조로 교체하고 승률을 선택 맵 반영 여부에 따라 바뀌는 한 줄로 통합했다. 이전 맵이나 일반 예측을 새 맵의 예측으로 재사용하지 않는다. 변경 파일 린트·타입 및 운영진/멤버의 실시간 확정·재접속·경기 시작 E2E 1건 통과. 표시 변경 범위라 DB·전체 빌드·전체 회귀는 생략했다.

**추가 수정 — 비교 위치·자동 참여:** M/A 토글을 출전 인원 표시 자리에 두고 팀 점수/일반 승률을 출전 카드 위에 통합했다. 맵별 승률은 맵 선택/투표 단계에만 노출한다. QA 전용 샘플 점수·승률은 저장하지 않으며 실제 점수 편집과 분리한다. 깜짝 생성자는 즉시/예약 모두 참여 처리하고 기존 활성 방의 누락 응답만 보완한다. 로비 버튼은 더보기 유무와 관계없이 같은 열에 배치한다. QA 62개·운영 50개(12개 대기), 생성 타입 동일·새 트리거 advisor 경고 없음. 린트·타입·빌드와 기존 집중 E2E 1건으로 점수/맵/샘플 비저장·생성자 참여·데스크톱/모바일 정렬을 검증했다. 계정 전환 쿠키와 모바일 캡처 대기만 보완했으며 앱 빌드를 반복하거나 QA 명단을 초기화하지 않았다.
