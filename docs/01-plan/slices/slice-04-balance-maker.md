...
- 수동 명단은 좌우 5칸, 각 팀 **딜·딜·탱·힐·힐** 고정과 기존 중앙 역할 아이콘을 사용한다. 관전자 영역·등록은 제외한다. 이름 클릭 → 1팀 1~5번·2팀 1~5번 순서의 첫 빈칸 입력, 자리 우클릭 → 비우기(2026-09-16 사용자 확정). 자리 선택·맞교환·반복 위치 문구·체크를 제거한다. 선택 여부는 배경색으로 구분하고 `편성하기` 행 왼쪽에 색상 범례를 둔다. 보조 입력은 `D-UX-01`에서 검토한다.
