...
**현재 시드**: 오버워치에 `QA_01_Clan` 1개 + 리더 `QA_Leader_01` 활성 멤버십. 해당 클랜은 시드 후 **`subscription_tier = premium`** 으로 갱신되어 **Premium 전용 기능**(예: 클랜 이벤트 대진표 초안 탭)·Playwright 회귀를 한 경로에서 검증할 수 있다.
